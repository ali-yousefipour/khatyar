...
ترتیب و عنوان امکانات Web App از این مرحله با `DashboardScreen.js` در Android همسان‌سازی شده است. لایه `php/app/web-android-parity-v2.js` نسخه 3 را اعمال می‌کند و به‌صورت runtime:
...
- روی صفحات احراز‌شده، کنترل عمومی نمایش/اندازه تصاویر و اجزای فرم را اعمال می‌کند.
- جریان دوربین برای ثبت تصاویر عملیاتی را به camera-only محدود می‌کند.
...
حداقل دقت ثبت ایستگاه: **۲۰ متر** و این محدودیت در Backend enforce می‌شود.

Migration `php/migrations/2026_08_26_line_location.sql` جداول و تمام ستون‌های موردنیاز جدول `lines` از جمله `station_name` را به‌صورت idempotent ایجاد می‌کند و برای سازگاری بیشتر، اضافه‌کردن ستون‌ها را با `INFORMATION_SCHEMA` انجام می‌دهد.
...
مدیریت کاربران، نقش‌ها، Permissionها، خطوط، ثبت موقعیت/تصویر، بازدید خطوط، گزارش‌های مدیریتی و تنظیمات سامانه. دسترسی `can_capture/can_view/can_manage` از API مرکزی کنترل می‌شود.

`line-location-admin-bridge.js` نمایش اطلاعات موقعیت، دقت، تصاویر و تاریخچه را به رکورد خط متصل می‌کند و `line-location-admin-module.js` ویرایش همین Permissionها را داخل بخش تنظیمات پنل ارائه می‌دهد. خروجی Excel خطوط نیز از UI پنل به `line-location-export.php` متصل است.
...
Migration ثبت موقعیت خطوط به همین الزام پایبند است و از `ADD COLUMN IF NOT EXISTS` وابسته به نسخه استفاده نمی‌کند.
...
## 10. تغییرات 2026-08-26 — تکمیل مرحله کدنویسی
...
- حداقل دقت ۲۰ متر در Backend enforce شد.
- Migration ثبت موقعیت خطوط بازبینی شد و `station_name` و سایر ستون‌های لازم `lines` به آن اضافه شدند.
- Permissionهای ثبت/مشاهده/مدیریت در تنظیمات پنل نیز به‌صورت native module در دسترس قرار گرفتند.
- خروجی Excel خطوط به UI پنل متصل است.
- وضعیت کارهای باقی‌مانده فقط بر اساس کد تأییدشده سبز شده و تست دستگاه/Production عمداً به‌عنوان وابسته به محیط باقی مانده است.
