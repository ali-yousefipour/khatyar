...
adminRouter.delete("/geofences/:id", requireAdmin, async (req, res) => {
  await q(`DELETE FROM geofences WHERE id=$1`, [req.params.id]);
  res.json({ ok: true });
});
...
adminRouter.post("/messages", requireAdmin, async (req, res) => {
  ...
  let ids = [];
  if (b.target_type === "selected")
    ids = [...new Set((b.user_ids || []).map(Number))];
  else if (b.target_type === "zone")
    ids = (
      await q(`SELECT id FROM users WHERE is_active AND zone_id=$1`, [
        b.zone_id,
      ])
    ).rows.map((r) => r.id);
  else
    ids = (
      await q(`SELECT id FROM users WHERE is_active AND id<>$1`, [req.user.id])
    ).rows.map((r) => r.id);
  if (!ids.length) return res.status(400).json({ error: "گیرنده‌ای یافت نشد" });
  const { rows } = await q(
    `INSERT INTO messages(sender_id,title,body,target_type,zone_id) VALUES ($1,$2,$3,$4,$5) RETURNING id`,
    [
      req.user.id,
      b.title ?? null,
      b.body,
  ...
  await notifyUsers(ids, b.title || "پیام جدید", String(b.body).slice(0, 120), {
    type: "message",
    message_id: mid,
  });
...
adminRouter.get("/messages/:id/receipts", requireAdmin, async (req, res) => {
  const { rows } = await q(
    `SELECT u.id, (u.first_name||' '||u.last_name) AS name, r.title AS role, mr.read_at
     ...
     WHERE mr.message_id=$1 ORDER BY (mr.read_at IS NULL) DESC, mr.read_at DESC`,
    [req.params.id]
  );
  res.json(rows);
});
...
adminRouter.get("/official-visits", requireAdmin, async (req, res) => {
  ...
  if (official) {
    ...
    conds.push(`ov.official_id=$${params.length}`);
  }
  if (from) {
    params.push(from);
    conds.push(`ov.created_at >= $${params.length}`);
  }
  if (to) {
    params.push(to);
    conds.push(`ov.created_at <= $${params.length}`);
  ...
  const { rows } = await q(
    `SELECT ov.id, ov.created_at, ov.note,
     ...
     ${where} ORDER BY ov.created_at DESC LIMIT 1000`,
    params
  );
  res.json(rows);
});
...
adminRouter.get("/stats", async (_req, res) => {
  const [drivers, lines, todayAtt, unpaid, notices] = await Promise.all([
    ...
    q(
      `SELECT count(*)::int n FROM attendances WHERE created_at::date = now()::date`
    ),
    q(`SELECT count(*)::int n FROM bills WHERE status <> 'پرداخت شده'`),
    q(
      `SELECT count(*)::int n FROM notices WHERE date_trunc('month',created_at)=date_trunc('month',now())`
    ),
  ]);
  const week = await q(
  ...
  res.json({
    ...
    lines: lines.rows[0].n,
    today_attendance: todayAtt.rows[0].n,
    unpaid_bills: unpaid.rows[0].n,
    notices_month: notices.rows[0].n,
    week_attendance: week.rows,
    by_line: byLine.rows,
  });
});
...
adminRouter.get("/report", async (req, res) => {
  ...
  try {
    if (type === "attendance") {
      ...
      return res.json({
        cols: ["تاریخ", "راننده", "خط", "ثبت‌کننده"],
    ...
    if (type === "notices") {
      ...
      return res.json({
        cols: ["تاریخ", "راننده", "موضوع", "اولویت", "ثبت‌کننده"],
        rows: rows.map((r) => [
          String(r.d),
          r.driver,
          r.reason || "",
          P[r.priority] || r.priority,
          r.by_user,
        ]),
      });
    }
    ...
    if (type === "bills") {
      ...
      const { rows } = await q(sql, params);
      return res.json({
        cols: ["شخص", "کد ملی", "پلاک", "مبلغ(ریال)", "وضعیت"],
        rows: rows.map((r) => [
          r.person_title,
          r.national_id,
          r.plate,
...
adminRouter.get("/roles", async (_req, res) =>
  res.json(
    (await q(`SELECT id,title,level,is_admin FROM roles ORDER BY level`)).rows
  )
);
...
adminRouter.post("/users", requireAdmin, async (req, res) => {
  const s = z.object({
    ...
    rank_stars: z.number().min(0).max(5).nullable().optional(),
    password: z.string().min(6).default("123456"),
  });
  const p = s.safeParse(req.body);
  if (!p.success) return res.status(400).json({ error: "ورودی نامعتبر" });
  const exists = await q(`SELECT 1 FROM users WHERE username=$1`, [
    p.data.username,
  ]);
  if (exists.rows[0])
  ...
  const { rows } = await q(
    ...
    [
      ...
      p.data.role_id,
      p.data.zone_id ?? null,
      p.data.manager_id ?? null,
      p.data.phone ?? null,
      p.data.rank_stars ?? null,
    ]
  );
  res.status(201).json(rows[0]);
});
...
adminRouter.get("/users/:id/lines", async (req, res) => {
  const { rows } = await q(
    `SELECT l.id, l.code, l.origin, l.destination FROM user_lines ul
     JOIN lines l ON l.id=ul.line_id WHERE ul.user_id=$1`,
    [req.params.id]
  );
  res.json(rows);
});
...
adminRouter.post("/zones", requireAdmin, async (req, res) => {
  ...
  const { rows } = await q(
    `INSERT INTO zones(name,parent_id) VALUES ($1,$2) RETURNING *`,
    [name, parent_id ?? null]
  );
  res.status(201).json(rows[0]);
});
adminRouter.put("/zones/:id", requireAdmin, async (req, res) => {
  await q(`UPDATE zones SET name=$1 WHERE id=$2`, [
    req.body.name,
    req.params.id,
  ]);
  res.json({ ok: true });
...
adminRouter.get("/lines", async (_req, res) =>
  res.json(
    (
      await q(
        `SELECT id,code,origin,destination,status FROM lines ORDER BY code`
      )
    ).rows
  )
);
...
adminRouter.post("/notice-reasons", requireAdmin, async (req, res) => {
  const { rows } = await q(
    `INSERT INTO notice_reasons(title) VALUES ($1) RETURNING *`,
    [req.body.title]
  );
  res.status(201).json(rows[0]);
});
adminRouter.delete("/notice-reasons/:id", requireAdmin, async (req, res) => {
  await q(`UPDATE notice_reasons SET is_active=false WHERE id=$1`, [
    req.params.id,
  ]);
  res.json({ ok: true });
});
...
adminRouter.post("/checklist-templates", requireAdmin, async (req, res) => {
  ...
  if (!p.success) return res.status(400).json({ error: "ورودی نامعتبر" });
  const t = await q(
    `INSERT INTO checklist_templates(title) VALUES ($1) RETURNING id`,
    [p.data.title]
  );
