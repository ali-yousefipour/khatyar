...
const _cache = {};
const _cacheTTL = { '/admin/users': 60000, '/lines': 120000, '/geofences': 120000, '/zones': 120000, '/notice-reasons': 300000 };
function _invalidateCache(){ for(const k in _cache) delete _cache[k]; }
async function GET(p, opts){
  const ttl = (opts&&opts.ttl) || _cacheTTL[p];
  if(ttl && _cache[p] && (Date.now()-_cache[p].t)<ttl) return _cache[p].v;
  const r = await fetch(API_BASE+p,{headers:tok()});
  if(!r.ok) throw new Error((await r.json().catch(()=>({}))).error||'خطای سرور');
  const v = await r.json();
  if(ttl) _cache[p]={t:Date.now(),v};
  return v;
}
...
const db = {
  login: async (u,p)=>{ const d=await SEND('POST','/auth/login',{username:u,password:p,device_id:'web-panel'}); localStorage.token=d.access; return d; },
...
function JDate({value,onChange,placeholder,jalali}){ const [open,setOpen]=useState(false);
  // value می‌تواند رشتهٔ ISO میلادی («YYYY-MM-DD») یا رشتهٔ جلالی («YYYY/MM/DD») باشد.
  // اگر jalali=true باشد، خروجی onChange به‌صورت جلالی «YYYY/MM/DD» خواهد بود؛ وگرنه ISO میلادی.
  const parseInit=()=>{
    if(value && typeof value==="string"){
      if(value.indexOf("/")>=0){ const[y,m,d]=value.split("/").map(Number); if(y&&m&&d) return [y,m,d]; }
      else if(value.indexOf("-")>=0){ const[y,m,d]=value.split("-").map(Number); if(y&&m&&d) return gregToJalali(y,m,d); }
    }
    return todayJ();
  };
  ...
  const label=()=>{ if(!value)return""; try{ if(typeof value==="string" && value.indexOf("/")>=0) return value; return jLabel(value); }catch(e){ return ""; } };
  return(<span style={{position:"relative",display:"inline-block"}}>
    ...
    {open&&<div style={{position:"absolute",zIndex:9999,background:"#fff",border:"1px solid var(--line)",borderRadius:12,padding:10,boxShadow:"0 8px 24px rgba(0,0,0,.15)",top:"110%",right:0,width:240}}>
      <div className="row" style={{justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        <button className="btn g" onClick={()=>{ if(vm===1){setVm(12);setVy(vy-1);}else setVm(vm-1); }}>‹</button>
        <b style={{fontSize:13}}>{J_MONTHS[vm-1]} {vy}</b>
        <button className="btn g" onClick={()=>{ if(vm===12){setVm(1);setVy(vy+1);}else setVm(vm+1); }}>›</button></div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
        {Array.from({length:daysIn(vy,vm)},(_,i)=>i+1).map(d=><button key={d} className="btn g" style={{padding:"5px 0",fontSize:12}} onClick={()=>pick(d)}>{d}</button>)}</div>
        ...
        {bdays.people.map((p,i)=>{
          ...
          return <div key={i} style={{background:"#fff",borderRadius:10,padding:"10px 14px",border:p.is_today?"2px solid #16a06a":"1px solid #f0e0e8",minWidth:200,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:10.5,color:"#b08",direction:"ltr",textAlign:"right"}}>{fa(p.birth_date)}</div>
            </div>
...
function LiveMap(){
  ...
  return(<div className="panel"><h3>نقشهٔ نیروها و رهگیری
    ...
    <div className="tabbar" style={{display:"flex",gap:8,marginBottom:12,borderBottom:"2px solid var(--line)",flexWrap:"wrap"}}>
      {[["main","🗺 نقشهٔ اصلی نیروها"],["live","🛰 رهگیری زنده"],["report","📊 گزارش مسیر و پخش"],["exits","⛔ خروج از محدوده امروز"],["gsm","📶 موقعیت‌های GSM"]].map(([k,lbl])=>
    ...
    {viewTab==="gsm"&&<div style={{marginTop:6}}>
      <p style={{fontSize:13,color:"var(--muted)",marginBottom:8,padding:"0 4px"}}>📶 موقعیت‌هایی که هنگام خاموش بودن GPS کاربر، از طریق آنتن مخابراتی (GSM) به‌صورت حدودی ثبت شده‌اند. این موقعیت‌ها مکمل GPS هستند و روی نقشهٔ اصلی هم با نشان 📶 مشخص‌اند.</p>
      <GsmReport/>
    </div>}
  </div>);
}
...
function Users(){
  ...
  return(<div className="panel"><h3>مدیریت کاربران <button className="btn p" onClick={()=>setAdding(true)}>+ افزودن کاربر</button> <button className="btn g" onClick={async()=>{ try{ const res=await fetch(db.usersExportUrl(),{headers:tok()}); if(!res.ok)throw new Error("خطا"); const blob=await res.blob(); const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="اطلاعات_کاربران.csv"; a.click(); }catch(e){alert(e.message);} }}>⤓ خروجی کامل کاربران</button></h3>
    ...
    <table><thead><tr><th></th><th>نام</th><th>کد ملی</th><th>سمت</th><th>ایمیل</th><th>تعهدات</th><th>دستگاه‌ها</th><th>وضعیت</th><th>اقدامات</th></tr></thead>
    <tbody>{pg.slice.map(u=><tr key={u.id}>
      ...
      <td>{u.role_title||rt(u.role_id)}</td><td style={{direction:"ltr",textAlign:"right",fontSize:12}}>{u.email||"—"}</td>
    ...
    {edit&&<Modal title="ویرایش کاربر" onClose={()=>{setEdit(null);setPw("");}}>
      ...
      <div className="row" style={{marginTop:6,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{flex:1,minWidth:170}}><label>تاریخ تولد</label><JDate value={edit.birth_iso||(edit.birth_date?isoFromJ(...edit.birth_date.split("-").map(Number)):"")} onChange={v=>{ let bd=""; if(v){ const [y,m,d]=v.split("-").map(Number); const [jy,jm,jd]=gregToJalali(y,m,d); bd=`${jy}-${String(jm).padStart(2,"0")}-${String(jd).padStart(2,"0")}`; } setEdit({...edit,birth_iso:v,birth_date:bd}); }}/>{edit.birth_date&&<span className="muted" style={{fontSize:11}}>{edit.birth_date.replace(/-/g,"/")}</span>}</div>
      </div>
      <CustomFieldsFiller userId={edit.id}/>
...
function AddUser({roles,zones,onClose,onSave}){
  const [f,setF]=useState({first_name:"",last_name:"",username:"",email:"",role_id:roles[0]?.id||9,zone_id:null,password:"123456",allow_android:1,allow_web:1});
...
function OrgChart(){
  const [users,setUsers]=useState([]); const [drag,setDrag]=useState(null); const [over,setOver]=useState(null); const [multiUser,setMultiUser]=useState(null);
  const reload=()=>db.users().then(setUsers).catch(()=>{});
  useEffect(()=>{reload()},[]);
  const setMgr=async(uid,mid)=>{ await db.setOrg(uid,{manager_id:mid||null}); reload(); };
  const isDesc=(anc,node)=>{ let c=node; const byId=id=>users.find(u=>u.id===id); while(c){ if(c.manager_id===anc.id)return true; c=byId(c.manager_id); } return false; };
  const onDrop=async(target)=>{ const d=drag; setDrag(null); setOver(null);
    if(!d||d.id===target.id) return;
    if(isDesc(d,target)){ alert("نمی‌توان یک فرد را زیرمجموعهٔ زیردستان خودش کرد."); return; }
    await setMgr(d.id, target.id);
  };
  const roots=users.filter(u=>!u.manager_id);
  const Node=u=>{ const kids=users.filter(c=>c.manager_id===u.id);
    return(<div key={u.id} className="treenode">
      <div className="treebox" draggable
        onDragStart={()=>setDrag(u)} onDragOver={e=>{e.preventDefault();setOver(u.id);}} onDragLeave={()=>setOver(o=>o===u.id?null:o)}
        onDrop={e=>{e.preventDefault();onDrop(u);}}
        style={{cursor:"grab",outline:over===u.id?"2px dashed var(--brand)":"none",opacity:drag&&drag.id===u.id?.5:1}}>
        <b>{u.first_name} {u.last_name}</b><small>{u.role_title}</small>
        <select value="" onChange={e=>{if(e.target.value)setMgr(u.id, e.target.value==="0"?null:+e.target.value);}} style={{marginTop:6,fontSize:11,width:"100%"}}>
          <option value="">تغییر سرپرست…</option><option value="0">بدون سرپرست</option>
          {users.filter(m=>m.id!==u.id).map(m=><option key={m.id} value={m.id}>{m.first_name} {m.last_name}</option>)}</select>
        <button className="btn g" style={{marginTop:4,fontSize:11,width:"100%",padding:"3px 0"}} onClick={()=>setMultiUser(u)}>مقام‌های بالاسری متعدد…</button>
      </div>
      {kids.length>0&&<div className="treekids">{kids.map(Node)}</div>}
    </div>); };
  return(<div className="panel"><h3>چارت سازمانی (درختی — از بالا به پایین)</h3>
    <p className="muted" style={{marginBottom:12}}>هر فرد را با کشیدن (Drag) و رها کردن روی فرد دیگر، زیرمجموعهٔ او کنید؛ یا از منوی «تغییر سرپرست» استفاده کنید. برای حذف سرپرست، گزینهٔ «بدون سرپرست» را انتخاب کنید.</p>
    <div className="treewrap"><div className="treerow"
      onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault(); if(drag){setMgr(drag.id,null);setDrag(null);}}}>{roots.map(Node)}</div></div>
...
function Config(){
  ...
  const TABS=[["define","تعریف موضوعات و چک‌لیست"],["notices","تذکرات داده‌شده"],["checklists","چک‌لیست‌های داده‌شده"]];
  return(<div>
...
function GivenChecklists(){
  ...
  return(<div className="panel"><h3>چک‌لیست‌های داده‌شده</h3>
    ...
    <div style={{overflowX:"auto"}}><table style={{fontSize:12,minWidth:820}}><thead><tr><th>تاریخ</th><th>راننده</th><th>کد ملی</th><th>خط</th><th>چک‌لیست</th><th>پاسخ‌ها</th><th>ثبت‌کننده</th><th>سمت</th></tr></thead>
      ...
      {!rows.length&&!loading&&<tr><td colSpan="8" className="muted" style={{textAlign:"center"}}>چک‌لیستی یافت نشد</td></tr>}</tbody></table></div>
  </div>);
}
...
function ConfigDefine(){
  ...
  const saveCl=async()=>{
    const payload=items.map(i=>({label:i.label, answer_type:i.answer_type||"single", options:(i.optStr||"").split(/[،,]/).map(s=>s.trim()).filter(Boolean)}));
    await db.saveChecklist("چک‌لیست بازدید خودرو", payload); alert("چک‌لیست ذخیره شد.");
  };
    ...
    {subsFor&&<Modal title={"پاسخ‌های فرم: "+subsFor.form.title} onClose={()=>setSubsFor(null)}>
      ...
      <div style={{overflowX:"auto"}}><table style={{fontSize:11.5,minWidth:600}}><thead><tr><th>تاریخ</th><th>ثبت‌کننده</th><th>راننده</th>{(subsFor.form.schema||[]).map((c,i)=><th key={i}>{c.label}</th>)}</tr></thead><tbody>
        {subsFor.submissions.map(sb=><tr key={sb.id}><td style={{whiteSpace:"nowrap"}}>{fj(sb.created_at)}</td><td>{sb.by_name}</td><td>{(sb.driver_name||"").trim()||"—"}</td>
          {(subsFor.form.schema||[]).map((c,i)=>{ let val=sb.answers?sb.answers[c.key]:""; if(Array.isArray(val))val=val.join("، "); if(typeof val==="string"&&val.startsWith("data:image"))val="[تصویر]"; return <td key={i}>{val||"—"}</td>; })}
        </tr>)}
      </tbody></table></div>}
      ...
      {rep&&<>
        ...
        <div style={{overflowX:"auto",marginTop:14}}><table style={{fontSize:12.5,minWidth:720}}>
          ...
          <tbody>{rep.rows.map(r=><tr key={r.id}>
            <td style={{fontWeight:700}}>{r.type_title}</td><td style={{fontSize:11.5}}>{r.place_title||r.location||"—"}</td><td>{r.driver_name||"—"}</td><td dir="ltr">{fa(r.driver_national_id)}</td>
    ...
    {tab==="report"&&<div className="panel">
      ...
      {rep&&<>
        ...
        <div style={{overflowX:"auto",marginTop:14}}><table style={{fontSize:12.5,minWidth:760}}>
          ...
          <tbody>{rep.rows.map(r=><tr key={r.id}>
            <td style={{fontWeight:700}}>{r.item_title}</td><td style={{fontSize:11.5}}>{r.place_title||"—"}</td><td>{r.driver_name||"—"}</td><td dir="ltr">{fa(r.driver_national_id)}</td>
...
function OrgBrandingSettings({v,set,save}){
  ...
  const pickLogo=(e)=>{ const f=e.target.files&&e.target.files[0]; if(!f)return;
    if(f.size>500*1024){ alert("حجم لوگو باید کمتر از ۵۰۰ کیلوبایت باشد."); return; }
    const r=new FileReader(); r.onload=()=>set("org_logo",r.result); r.readAsDataURL(f); };
  return(<div>
    <p style={{fontSize:13,color:"var(--muted)",marginBottom:10}}>عنوان اداره و لوگو در سربرگ چاپ گزارش‌ها و خروجی اکسل‌ها درج می‌شوند.</p>
    <label className="label">عنوان اداره</label>
    ...
    <label className="label" style={{marginTop:10}}>لوگوی سازمان</label>
    <div className="row" style={{gap:12,alignItems:"center",marginTop:4}}>
      {v.org_logo? <img src={v.org_logo} alt="لوگو" style={{height:64,width:64,objectFit:"contain",border:"1px solid var(--line)",borderRadius:10,background:"#fff",padding:4}}/> : <div style={{height:64,width:64,border:"1px dashed var(--line)",borderRadius:10,display:"grid",placeItems:"center",color:"var(--muted)",fontSize:11}}>بدون لوگو</div>}
      <input type="file" accept="image/*" ref={fileRef} style={{display:"none"}} onChange={pickLogo}/>
      <button className="btn g" onClick={()=>fileRef.current&&fileRef.current.click()}>انتخاب لوگو</button>
      {v.org_logo&&<button className="btn g" onClick={()=>set("org_logo","")}>حذف لوگو</button>}
    </div>
...
function Settings(){
  ...
  const TABS=[["general","عمومی و موقعیت"],["dashboard","داشبورد و محاسبهٔ عملکرد"],["hr","منابع انسانی"],["fields","فیلدهای پرسنل"],["appitems","آیتم‌های اپ هر سمت"],["sms","پیامک"],["security","امنیت و نسخه اپ"],["files","پیوست‌ها و اعلان‌ها"],["drivers","بدهکاران"],["print","قالب چاپ"],["access","دسترسی‌ها"],["backup","پشتیبان‌گیری و پاکسازی"]];
  ...
  return(<div>
    ...
    <div className="panel t-general"><h3>📶 ردیابی موقعیت از طریق آنتن GSM</h3>
      <p style={{fontSize:12.5,color:"var(--muted)",marginBottom:10}}>وقتی GPS کاربر خاموش باشد، برنامه موقعیت حدودی را از آنتن مخابراتی (GSM) ارسال می‌کند. این موقعیت‌ها در نقشهٔ زنده با نشان «موقعیت حدودی» و در «گزارش موقعیت GSM» نمایش داده می‌شوند.</p>
      <label className="row" style={{gap:8,marginBottom:8}}><input type="checkbox" checked={v.gsm_tracking_enabled!==false} onChange={e=>set("gsm_tracking_enabled",e.target.checked)}/><b>ارسال موقعیت حدودی از طریق GSM فعال باشد</b></label>
      <label className="label">فاصلهٔ زمانی ارسال موقعیت GSM (دقیقه)</label>
      <input className="input" type="number" min="1" max="60" style={{maxWidth:120}} value={v.gsm_ping_minutes||2} onChange={e=>set("gsm_ping_minutes",parseInt(e.target.value)||2)}/>
      <label className="label" style={{marginTop:10}}>فاصلهٔ بررسی روشن بودن موقعیت GPS (ثانیه)</label>
      <input className="input" type="number" min="15" max="600" style={{maxWidth:120}} value={v.gps_check_seconds||60} onChange={e=>set("gps_check_seconds",parseInt(e.target.value)||60)}/>
      <button className="btn p" style={{marginTop:10,display:"block"}} onClick={save}>ذخیره</button>
    </div>
    <div className="panel t-general"><h3>لوگو و عنوان اداره (برای چاپ و خروجی اکسل)</h3>
    ...
    <div className="panel t-security"><h3>قوانین امنیتی ورود</h3>{Toggle("require_gps","الزام روشن‌بودن GPS")}
      ...
      <p style={{fontSize:12,color:"var(--muted)",marginTop:10}}>این قوانین توسط اپ موبایل بررسی و توسط سرور اعمال می‌شوند.</p></div>
    ...
    <div className="panel t-fields"><h3>🗂 فیلدهای سفارشی پرسنل</h3><CustomFieldsManager/></div>
    ...
    <div className="panel t-security"><h3>📍 اعلان خروج کاربر از محدوده خط</h3>
      <p className="muted" style={{fontSize:13,marginBottom:10}}>با فعال‌سازی این گزینه، وقتی موقعیت ارسالی اپ نشان دهد کاربر از محدودهٔ ایستگاه/خط تعریف‌شده خارج شده، برای مسئول بالادستی یا شخص منتخب اعلان داخل برنامه ارسال می‌شود.</p>
      {(()=>{const cur=v.station_exit_notify||{}; const setSub=(k,val)=>set("station_exit_notify",{enabled:!!cur.enabled,mode:cur.mode||"hierarchy",user_id:cur.user_id||null,[k]:val}); return <>
        <label className="row" style={{gap:8,marginBottom:10}}><input type="checkbox" checked={!!cur.enabled} onChange={e=>setSub("enabled",e.target.checked)}/>ارسال خروج از محدوده کاربر به مسئول بالادستی</label>
        {cur.enabled&&<div style={{marginInlineStart:8}}>
          <label className="label">نوع ارسال اعلان</label>
          <select className="input" value={cur.mode||"hierarchy"} onChange={e=>setSub("mode",e.target.value)} style={{maxWidth:260}}>
            <option value="hierarchy">بر اساس سلسله‌مراتب تعریف‌شده</option>
            <option value="specific">ارسال برای یک شخص مشخص</option>
          </select>
          {cur.mode==="specific"&&<div style={{marginTop:8}}>
            <label className="label">شناسه کاربر دریافت‌کننده اعلان</label>
            <input className="input" type="number" value={cur.user_id||""} onChange={e=>setSub("user_id",+e.target.value||null)} style={{maxWidth:180}} placeholder="User ID"/>
            <p className="muted" style={{fontSize:12,marginTop:4}}>شناسه کاربر را از بخش مدیریت کاربران بردارید.</p>
          </div>}
        </div>}
      </>})()}
      <button className="btn p" style={{marginTop:10}} onClick={save}>ذخیره تنظیمات خروج از محدوده</button>
    </div>
    ...
    <div className="panel t-dashboard"><h3>حذف خودکار تصاویر و پیوست‌ها</h3>
      ...
      <button className="btn p" style={{marginTop:8}} onClick={save}>ذخیره</button>
      <p style={{fontSize:12,color:"var(--muted)",marginTop:10}}>تصاویر/پیوست‌های قدیمی‌تر از مدت تعیین‌شده، به‌صورت خودکار از سرور حذف می‌شوند. برای اجرای خودکار روزانه، یک Cron به آدرس <code dir="ltr">/api/cron/cleanup?key=کلید</code> تنظیم کنید.</p></div>
...
function Messages(){
  ...
  const [title,setTitle]=useState(""); const [body,setBody]=useState(""); const [receipts,setReceipts]=useState(null); const [sending,setSending]=useState(false); const [att,setAtt]=useState(null); const [alsoSms,setAlsoSms]=useState(false);
  ...
  const send=async()=>{ if(!body)return alert("متن پیام را وارد کنید."); setSending(true);
    try{ const r=await db.sendMessage({title,body,target_type:target,zone_id:zoneId?+zoneId:null,user_ids:sel,attachment_name:att?.name,attachment_data:att?.data,also_sms:alsoSms});
      alert(`پیام به ${fa(r.recipients||0)} نفر ارسال شد.`+(alsoSms?` پیامک به ${fa(r.sms_sent||0)} شماره ارسال شد.`:"")); setTitle("");setBody("");setSel([]);setAtt(null);setAlsoSms(false);setTab("sent");reload();
...
function PresenceSettings(){
  const [cfg,setCfg]=useState({enabled:false,slots:[],window_minutes:1,grace_minutes:15,alarm:true});
  const [nt,setNt]=useState(""); const [saved,setSaved]=useState(false);
  useEffect(()=>{ db.settings().then(s=>{ if(s.presence_check) setCfg({enabled:!!s.presence_check.enabled,slots:s.presence_check.slots||[],window_minutes:s.presence_check.window_minutes||1,grace_minutes:s.presence_check.grace_minutes||15,alarm:s.presence_check.alarm!==false}); }).catch(()=>{}); },[]);
  ...
  return(<div>
    ...
    <p style={{fontSize:13,color:"var(--muted)",marginBottom:10}}>در هر بازهٔ ساعتی تعریف‌شده، برای کاربرانِ «مشمول صحت‌سنجی» (در ویرایش کاربر) پنجره‌ای در اپ باز می‌شود تا سلفی و عکس خودروهای خط ارسال کنند.</p>
...
function DashboardConfig(){
  const GROUPS=[["supervisor","ناظر خط"],["operator","اپراتور"],["admin_staff","نیروی اداری"],["chief_inspector","سربازرس"],["inspector","بازرس"]];
  const DEF=()=>({groups:Object.fromEntries(GROUPS.map(([k])=>[k,{web:true,app:true,count:k==="chief_inspector"?3:5}])),zone_scope:"all"});
  const [roles,setRoles]=useState([]); const [cfg,setCfg]=useState({default:DEF(),roles:{}});
  ...
  useEffect(()=>{ db.roles().then(rs=>setRoles(rs||[])).catch(()=>{});
    db.settings().then(s=>{ if(s.dashboard_config){ const d=s.dashboard_config; const rr={}; for(const id in (d.roles||{})) rr[id]=migrate(d.roles[id]); setCfg({default:migrate(d.default)||DEF(),roles:rr}); } }).catch(()=>{}); },[]);
  const cur=()=> sel==="default" ? cfg.default : (cfg.roles[sel]||DEF());
  ...
  return(<div>
    <p style={{fontSize:13,color:"var(--muted)",marginBottom:10}}>برای هر سمت تعیین کنید در داشبورد او کدام گروه‌های پرکار/کم‌کار، با چه تعدادی، در «پنل وب» و در «اپلیکیشن» نمایش داده شوند و محدودهٔ محاسبه چه باشد. «پیش‌فرض» برای سمت‌هایی که تنظیم اختصاصی ندارند اعمال می‌شود.</p>
    <div className="row" style={{gap:8,marginBottom:14,flexWrap:"wrap"}}>
...
function RolePerms(){
  const MENU=[["dashboard","داشبورد"],["map","نقشهٔ زنده"],["present","آمار حاضرین"],["presentchart","نمودار زندهٔ حاضرین"],["officials","حضور مسئولین"],["covertselfies","سلفی‌های نامحسوس"],["messages","پیام‌رسانی"],["users","کاربران"],["zones","منطقه‌بندی"],["org","چارت سازمانی"],["drivers","رانندگان"],["lines","خطوط"],["bills","آبونمان"],["config","تذکر/چک‌لیست"],["forms","فرم‌ساز"],["reports","گردش گزارش"],["report","گزارش‌گیری"],["perfreport","گزارش عملکرد پرسنل"],["welfare","رفاهیات روابط عمومی"],["cultural","فعالیت‌های فرهنگی"],["gsmreport","گزارش موقعیت GSM"],["excel","ورود اکسل"],["logs","لاگ"],["useract","فعالیت کاربران"],["commitments","تعهدات انضباطی"],["tempdrivers","رانندگان موقت"],["presence","صحت‌سنجی حضور"],["attendance","حضور نیروها"],["shifts","شیفت و کارکرد"],["attreport","گزارش تردد پرسنل"],["workpolicy","سیاست کاری"],["requests","گزارش درخواست‌ها"],["outages","قطعی سیستم نوبت‌دهی"],["customfields","فیلدهای سفارشی"],["sms","ارسال پیامک"],["smslog","تاریخچهٔ پیامک"],["appitems","آیتم‌های اپ هر سمت"],["settings","تنظیمات"]];
...
function Reports(){
  ...
  const printReport=()=>{ const r=cur; const T=tpl||{};
    ...
    const piece=(k)=>{ if(!vis(k))return "";
      ...
      if(k==="sender")  return `<div class="meta">فرستنده: ${E((r.first_name||"")+" "+(r.last_name||""))}</div>`;
      ...
      if(k==="trail")   return `<h3>روند گردش</h3><table><thead><tr><th>اقدام</th><th>توسط</th><th>تاریخ</th><th>یادداشت</th></tr></thead><tbody>${(r.trail||[]).map(x=>`<tr><td>${x.action==="reply"?"پاسخ":x.action==="forward"?"ارجاع":"مشاهده"}</td><td>${E((x.a_first||"")+" "+(x.a_last||""))}</td><td>${E(fj(x.created_at))}</td><td>${E(x.note||"")}</td></tr>`).join("")}</tbody></table>`;
      return ""; };
  ...
  return(<div className="panel"><h3>گردش گزارش‌ها</h3>
    <div className="row" style={{gap:8,flexWrap:"wrap",marginBottom:12}}>
    ...
    <table><thead><tr><th>موضوع</th><th>فرستنده</th><th>وضعیت</th><th>تاریخ</th><th></th></tr></thead>
    ...
    {cur&&<Modal title={"گزارش: "+cur.subject} onClose={()=>setCur(null)}>
      ...
      {cur.attachment_url&&<a className="btn g" style={{marginTop:8,display:"inline-block"}} href="#" onClick={(e)=>{e.preventDefault();openMediaUrl(cur.attachment_url);}}>مشاهده/دریافت پیوست: {cur.attachment_name||"فایل"}</a>}
...
function ShiftManager(){
  ...
  const TABS=[["shifts","تعریف شیفت‌ها"],["assign","تخصیص به نیروها"],["holidays","تعطیلات رسمی"],["blocked","🚫 روزهای ممنوعهٔ مرخصی"],["leavebal","ماندهٔ مرخصی ابتدای دوره"],["report","گزارش کارکرد ماهانه"]];
  ...
  return(<div>
    ...
    <table><thead><tr><th>عنوان</th><th>نوع</th><th>اضافه‌کار روزانه/ماهانه</th><th></th></tr></thead><tbody>
      {list.map(s=><tr key={s.id}>
        <td>{s.title}</td>
        <td>{{simple:"ساده",advanced:"پیشرفته",floating:"شناور"}[s.type]||s.type}</td>
  ...
  return(<div>
    ...
    <div className="card-p" style={{marginBottom:14}}>
      <div className="row" style={{gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
        ...
        <div><label className="label">نوع</label><div style={{padding:"9px 0",fontWeight:700}}>{{simple:"ساده",advanced:"پیشرفته (تقویمی)",floating:"شناور"}[s.type]}</div></div>
      </div>
...
function AttendanceReport(){
  ...
  const exportExcel=()=>{ if(!data)return;
    const rows=[]; data.days.forEach(d=>{ if(!d.punches.length){ rows.push({"تاریخ":d.jdate,"روز هفته":d.weekday,"ورود":"--:--","خروج":"--:--","محل ورود":"--","محل خروج":"--","حضور در شیفت":d.in_shift,"کسری کار":d.shortage,"شب کاری":d.night,"اضافه کاری":d.overtime,"وضعیت":d.is_holiday?"تعطیل":(d.absent?"غیبت":"")}); }
      else d.punches.forEach((p,i)=>rows.push({"تاریخ":i===0?d.jdate:"","روز هفته":i===0?d.weekday:"","ورود":p.in||"--:--","خروج":p.out||"--:--","محل ورود":p.in_station||"--","محل خروج":p.out_station||"--","حضور در شیفت":i===0?d.in_shift:"","کسری کار":i===0?d.shortage:"","شب کاری":i===0?d.night:"","اضافه کاری":i===0?d.overtime:"","وضعیت":i===0&&d.is_holiday?"تعطیل":""})); });
  ...
  return(<div className="panel"><h3>🕒 گزارش تردد پرسنل</h3>
    ...
    {data&&<div style={{overflowX:"auto"}}>
      <table style={{fontSize:12.5,minWidth:640}}>
        <thead><tr><th>تاریخ</th><th>روز هفته</th><th>خلاصه تردد</th><th>حضور در شیفت</th><th>کسری کار</th><th>شب کاری</th><th>اضافه کاری</th></tr></thead>
...
function ShiftReport(){
  ...
  return(<div>
    ...
    <div style={{overflowX:"auto"}}><table style={{fontSize:12,minWidth:1040}}><thead><tr><th>نام و نام خانوادگی</th><th>موظفی</th><th>حضور در شیفت</th><th>کسری کار</th><th>غیبت</th><th>جمع غیبت و کسری</th><th>شب کاری</th><th>اضافه کاری</th><th>ماموریت</th><th>م.استحقاقی</th><th>م.استعلاجی</th></tr></thead><tbody>
      {rows.map((r,i)=>{ const gh=(r.absent_min||0)+(r.shortage||0); return(<tr key={i}><td style={{fontWeight:700}}>{r.name}</td><td>{hm(r.expected)}</td><td>{hm(r.worked)}</td><td style={{color:r.shortage?"var(--danger)":"inherit"}}>{hm(r.shortage)}</td><td style={{color:r.absent_min?"var(--danger)":"inherit"}}>{hm(r.absent_min||0)}</td><td>{hm(gh)}</td><td>{hm(r.night)}</td><td style={{color:"var(--ok)"}}>{hm(r.overtime)}</td><td>{hm(r.mission_min||0)}</td><td>{hm(r.annual_min||0)}</td><td>{hm(r.sick_min||0)}</td></tr>); })}
    </tbody></table></div>)}
  </div>);
}
...
function SmsSend(){
  const TABS=[["drivers","به رانندگان"],["bill_bulk","آبونمان (گروهی)"],["taxi_lic","پروانهٔ تاکسیرانی"],["op_lic","پروانهٔ بهره‌برداری"],["inspection","معاینهٔ فنی"],["insurance","بیمهٔ شخص ثالث"]];
  ...
  return(<div className="panel"><h3>✉ ارسال پیامک</h3>
    ...
    {tab==="drivers"?<SmsDrivers/>:tab==="bill_bulk"?<SmsBillBulk/>:<SmsExpiry type={tab} title={TABS.find(x=>x[0]===tab)[1]}/>}
  </div>);
}
...
function TempDriversPanel(){
  ...
  const lookup=async()=>{ const n=nid.replace(/\D/g,""); if(n.length<8){setMsg("کد ملی معتبر وارد کنید");return;}
    setMsg(""); setFound(null);
    try{ const r=await GET("/temp-drivers/search?national_id="+n); setFound(r); }
    catch(e){ setMsg(e.message||"راننده یافت نشد"); } };
  const add=async()=>{ if(!found?.driver){setMsg("ابتدا راننده را جستجو کنید");return;} if(!lineId){setMsg("خط ویژه را انتخاب کنید");return;}
    try{ await SEND("POST","/temp-drivers",{national_id:found.driver.national_id,line_id:+lineId,line_code_in_line:codeInLine||null,note:note||null});
      setMsg("✓ راننده به خط ویژه اضافه شد"); setLineId("");setCodeInLine("");setNote(""); lookup(); load(); }
  ...
  return(<div className="panel"><h3>🚕 رانندگان موقت خطوط ویژه</h3>
    ...
    <div className="card-p" style={{marginBottom:16}}>
      ...
      <div className="row" style={{gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
        <div><label className="label">کد ملی راننده</label><input className="input" style={{maxWidth:170}} value={nid} onChange={e=>setNid(e.target.value)} placeholder="کد ملی"/></div>
        <button className="btn p" onClick={lookup}>🔍 جستجو در همهٔ خطوط</button>
      </div>
      {found?.driver&&<div style={{marginTop:12,padding:12,background:"#f6f9fc",borderRadius:10}}>
        ...
        <span style={{color:"var(--muted)",marginRight:10,fontSize:12}}>کد ملی: {fa(found.driver.national_id)}</span>
    ...
    <div style={{overflowX:"auto"}}><table style={{fontSize:12.5,minWidth:720}}><thead><tr><th>نام</th><th>کد ملی</th><th>خط ویژه</th><th>کد در خط</th><th>موبایل</th><th>توضیح</th><th>افزوده توسط</th><th></th></tr></thead><tbody>
      {list.map(t=><tr key={t.id}>
        <td style={{fontWeight:700}}>{t.first_name} {t.last_name}</td><td style={{direction:"ltr",textAlign:"right"}}>{fa(t.national_id)}</td>
...
const VIEWS={
  ...
  gsmreport:{t:"گزارش موقعیت GSM",ic:"📶",c:GsmReport},
  ...
  return(<div style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"var(--paper)"}}>
    <div className="panel" style={{width:380}}>
      <div style={{textAlign:"center",marginBottom:18}}><div className="lg" style={{margin:"0 auto 10px",width:48,height:48,fontSize:22}}>ت</div>
        <h3 style={{justifyContent:"center"}}>ورود به سامانه مدیریت و نظارت بر خطوط و نیروهای اجرایی تاکسیرانی</h3></div>
...
function App(){
  const [me,setMe]=useState(null); const [v,setV]=useState("dashboard"); const [allowed,setAllowed]=useState(null); const [drawer,setDrawer]=useState(false);
  useEffect(()=>{ if(me&&me.is_admin){ db.settings().then(s=>{ const rp=(s.role_perms||{})[me.role_id]; setAllowed(rp&&rp.length?rp:null); }).catch(()=>setAllowed(null)); } },[me]);
  ...
  const CORE_VIEWS=["dashboard"];
  const vk=(!allowed||allowed.includes(v)||CORE_VIEWS.includes(v))?v:"dashboard"; const View=VIEWS[vk].c;
  const SECTIONS=[
    ["نمای کلی",["dashboard","map","present","presentchart"]],
    ["میدانی و گزارش‌ها",["officials","reports","report","perfreport","attreport","gsmreport","welfare","cultural","useract","outages","covertselfies"]],
    ["منابع انسانی",["presence","attendance","shifts","workpolicy","requests","commitments","customfields"]],
    ["پایگاه داده",["drivers","tempdrivers","lines","zones","bills","excel"]],
    ["پیام و سازمان",["messages","sms","smslog","org","users"]],
    ["تنظیمات و فرم‌ها",["config","forms","appitems","logs","settings"]],
  ];
  ...
  return(<div className={"layout"+(drawer?" drawer-open":"")}>
    ...
    <aside className="side"><div className="brand"><div className="lg">ت</div><span>سامانه مدیریت و نظارت تاکسیرانی</span></div>
      <nav className="nav" style={{flex:1}}>{SECTIONS.map(([title,keys])=>{ const ks=keys.filter(k=>VIEWS[k]&&can(k)); if(!ks.length)return null;
        return(<div key={title} className="navsec"><div className="navsec-t">{title}</div>
          {ks.map(k=><button key={k} className={v===k?"on":""} onClick={()=>closeOnPick(k)}><span className="ic">{VIEWS[k].ic}</span>{VIEWS[k].t}</button>)}</div>); })}
        <button onClick={()=>{localStorage.removeItem("token");setMe(null);}} style={{marginTop:8,color:"#ff9aa8"}}><span className="ic">⎋</span>خروج</button></nav>
...
(async ()=>{
  ...
  console.log("PANEL BUILD: 2026-06-21-v24 (covert-screenshot-build-fix, screenshot-settings)");
