...
// ارسال گروهی پیام در ربات‌های بله، تلگرام و ایتا
export default function BotMessageScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      request('/messengers/config').catch(() => null),
    ]).then(([cfg, l, q]) => {
      setTpls(cfg.templates || []); setTplOnly(!!cfg.templates_only);
      setLines(l || []);
      if (q) {
        setQuota(null); setCredit(null);
      }
  ...
  const send = async () => {
    if (!msg.trim()) { Alert.alert('خطا', 'متن پیام ربات را وارد یا قالبی انتخاب کنید'); return; }
    if (!totalCount) { Alert.alert('خطا', 'حداقل یک گیرنده انتخاب یا وارد کنید'); return; }
    Alert.alert('تأیید ارسال', `ارسال پیام در ربات‌ها به ${totalCount} گیرنده؟`, [
      { text: 'انصراف', style: 'cancel' },
      { text: 'ارسال', onPress: async () => {
        setBusy(true);
        try {
          const r = await request('/messengers/send-group', { method: 'POST', body: { driver_ids: chosen.map((d) => d.id), mobiles: manual, line_id: lineId ? Number(lineId) : null, role: roleGroup, message: msg.trim() } });
          Alert.alert('انجام شد', `در ربات‌ها ${r.sent || 0} ارسال انجام شد؛ ${r.not_connected || 0} مورد هنوز مشترک ربات نیستند.`);
          setSel({}); setManual([]);
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={{ padding: 14, paddingBottom: 40 }}>
      ...
      <Text style={s.label}>قالب پیام</Text>
      {tpls.length ? (
      ...
      <Text style={s.label}>متن پیام ربات{tplOnly ? ' (فقط از قالب‌ها قابل انتخاب است)' : ''}</Text>
      <TextInput style={[s.input, { height: 90, textAlignVertical: 'top' }, tplOnly && { backgroundColor: '#f1f1f4', color: C.muted }]} multiline value={msg}
        ...
        placeholder={tplOnly ? 'یک قالب را از بالا انتخاب کنید' : 'متن پیام ربات…'} placeholderTextColor={C.muted} />
      <Text style={s.muted}>پیام برای مشترکین متصل در بله، تلگرام و ایتا ارسال می‌شود.</Text>
          ...
          {loadingDrivers ? (
            ...
            <>
              <View style={s.summaryBox}>
                ...
                {!pickFromList && <Text style={s.summarySub}>پیام در ربات‌ها به همهٔ این رانندگان ارسال می‌شود.</Text>}
              </View>
      ...
      <TouchableOpacity style={[s.btn, busy && { opacity: 0.6 }]} disabled={busy} onPress={send}>
        <Text style={s.btnTxt}>{busy ? 'در حال ارسال…' : `ارسال در ربات‌ها به ${faNum(totalCount)} گیرنده`}</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
