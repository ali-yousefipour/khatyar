...
route('GET','/api/admin/inventory/ledger',function($p,$b,$u){
  ...
  $rows = Db::all("SELECT t.id,t.quantity,t.note,t.created_at,t.confirmed_at,t.transferable,it.name item_name,it.unit,
      TRIM(CONCAT(COALESCE(fu.first_name,''),' ',COALESCE(fu.last_name,''))) from_name,    FROM inventory_transfers t JOIN inventory_item_types it ON it.id=t.item_type_id
...
function _ssv_tables(){
  ...
  $sql=[
    "CREATE TABLE IF NOT EXISTS school_service_companies (
      ...
      status VARCHAR(30) NOT NULL DEFAULT 'فعال',
  ...
  // تکمیل ساختار پروفایل شرکت‌ها برای نصب‌های قدیمی
  foreach([
    "ALTER TABLE school_service_companies ADD COLUMN manager_mobile VARCHAR(30) NULL",
    "ALTER TABLE school_service_companies ADD COLUMN landline_phone VARCHAR(30) NULL",
    "ALTER TABLE school_service_companies ADD COLUMN latitude DECIMAL(10,7) NULL",
    "ALTER TABLE school_service_companies ADD COLUMN longitude DECIMAL(10,7) NULL",
    "ALTER TABLE school_service_companies ADD COLUMN registered_school_count INT NOT NULL DEFAULT 0",
    "ALTER TABLE school_service_companies ADD COLUMN representative_count INT NOT NULL DEFAULT 0",
  ...
  try{Db::run("ALTER TABLE school_service_schools ADD COLUMN latitude DECIMAL(10,7) NULL");}catch(Throwable $e){}
  try{Db::run("ALTER TABLE school_service_schools ADD COLUMN longitude DECIMAL(10,7) NULL");}catch(Throwable $e){}
  ...
  try{
    $roles=Db::all("SELECT id,level,title FROM roles");
    foreach($roles as $r){
      ...
      Db::run("INSERT INTO school_service_permissions(role_id,can_view,can_create,can_edit,can_delete,can_import,can_report)
        ...
        [(int)$r['id'],$default,$default,$default,$default,$default,$default]);
    }
  }catch(Throwable $e){}
  try{Db::run("UPDATE school_service_permissions p JOIN roles r ON r.id=p.role_id SET p.can_view=IF(r.title IN ('مدیر کل','مدیرکل','معاونت بازرسی','رییس اداره بازرسی','رئیس اداره بازرسی','سربازرس ارشد','نیروی اداری ارشد','سربازرس','بازرس','گشت خودرویی','گشت موتوری'),p.can_view,0),p.can_create=IF(r.title IN ('مدیر کل','مدیرکل','معاونت بازرسی','رییس اداره بازرسی','رئیس اداره بازرسی','سربازرس ارشد','نیروی اداری ارشد','سربازرس','بازرس','گشت خودرویی','گشت موتوری'),p.can_create,0),p.can_edit=IF(r.title IN ('مدیر کل','مدیرکل','معاونت بازرسی','رییس اداره بازرسی','رئیس اداره بازرسی','سربازرس ارشد','نیروی اداری ارشد','سربازرس','بازرس','گشت خودرویی','گشت موتوری'),p.can_edit,0),p.can_delete=IF(r.title IN ('مدیر کل','مدیرکل','معاونت بازرسی','رییس اداره بازرسی','رئیس اداره بازرسی','سربازرس ارشد','نیروی اداری ارشد','سربازرس','بازرس','گشت خودرویی','گشت موتوری'),p.can_delete,0),p.can_import=IF(r.title IN ('مدیر کل','مدیرکل','معاونت بازرسی','رییس اداره بازرسی','رئیس اداره بازرسی','سربازرس ارشد','نیروی اداری ارشد','سربازرس','بازرس','گشت خودرویی','گشت موتوری'),p.can_import,0),p.can_report=IF(r.title IN ('مدیر کل','مدیرکل','معاونت بازرسی','رییس اداره بازرسی','رئیس اداره بازرسی','سربازرس ارشد','نیروی اداری ارشد','سربازرس','بازرس','گشت خودرویی','گشت موتوری'),p.can_report,0)");}catch(Throwable $e){}
}
...
function _ssv_resolve_inspection_location($schoolId,$useSchoolLocation,$lat,$lng,$locationText){
  if(!$useSchoolLocation) return [$lat,$lng,$locationText];
  $schoolId=(int)$schoolId;
  if(!$schoolId) Http::error('برای استفاده از محل درب مدرسه، ابتدا مدرسه را انتخاب کنید.',422);
  $s=Db::one("SELECT id,name,latitude,longitude FROM school_service_schools WHERE id=? AND is_active=1",[$schoolId]);
  if(!$s) Http::error('مدرسه انتخاب‌شده معتبر نیست.',422);
  if($s['latitude']===null||$s['longitude']===null) Http::error('موقعیت این مدرسه هنوز ثبت نشده است. ابتدا موقعیت مدرسه را ثبت کنید.',422);
  return [(float)$s['latitude'],(float)$s['longitude'],'درب مدرسه — '.($s['name']??'مدرسه')];
}
...
function _ssv_sheet_records($rows){
  ...
  $aliases=[
    ...
    'manager_mobile'=>['تلفن همراه مدیرعامل','موبایل مدیرعامل','موبایل مدیر','موبایل'],
    'landline'=>['تلفن ثابت','تلفن شرکت','تلفن'],
    'phone'=>['تلفن شرکت','شماره تماس','موبایل'],
    'address'=>['آدرس','نشانی'],
    'latitude'=>['عرض جغرافیایی','latitude','lat'],
    'longitude'=>['طول جغرافیایی','longitude','lng'],
    'registered_school_count'=>['مدارس ثبت‌شده','تعداد مدارس ثبت‌شده'],
    'representative_count'=>['تعداد نمایندگان','نمایندگان'],
    'status'=>['وضعیت شرکت','وضعیت'],
    'profile_complete'=>['تکمیل پروفایل','وضعیت تکمیل پروفایل']
  ];
  ...
  for($r=1;$r<count($rows);$r++){ $x=$rows[$r];$get=fn($k)=>$idx[$k]===null?'':trim((string)($x[$idx[$k]]??''));$school=$get('school');$company=$get('company');if(in_array($company,['نامشخص','نامعلوم','-','—'],true))$company='';if($school===''&&$company==='')continue;$out[]=['code'=>$get('code'),'school'=>$school,'district'=>$get('district'),'gender'=>$get('gender'),'company'=>$company,'manager'=>$get('manager'),'manager_mobile'=>$get('manager_mobile'),'landline'=>$get('landline'),'phone'=>$get('phone'),'address'=>$get('address'),'latitude'=>$get('latitude'),'longitude'=>$get('longitude'),'registered_school_count'=>$get('registered_school_count'),'representative_count'=>$get('representative_count'),'status'=>$get('status'),'profile_complete'=>$get('profile_complete')];}
  return $out;
}
...
route('POST','/api/school-service/schools',function($p,$b,$u){_ssv_need($u,'edit');_ssv_tables();$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام مدرسه الزامی است',422);$g=in_array($b['gender']??'',['دخترانه','پسرانه'],true)?$b['gender']:'نامشخص';$cid=!empty($b['company_id'])?(int)$b['company_id']:0;if($cid&&!Db::one("SELECT id FROM school_service_companies WHERE id=? AND is_active=1",[$cid]))Http::error('شرکت مجری نامعتبر است',422);[$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);Db::run("INSERT INTO school_service_schools(code,name,educational_district,gender,address,latitude,longitude) VALUES(?,?,?,?,?,?,?)",[_ssv_norm($b['code']??'')?:null,$name,_ssv_norm($b['educational_district']??'')?:null,$g,_ssv_norm($b['address']??'')?:null,$lat,$lng]);$sid=(int)Db::pdo()->lastInsertId();if($cid)Db::run("INSERT INTO school_service_school_companies(school_id,company_id,is_primary) VALUES(?,?,1)",[$sid,$cid]);return ['ok'=>true,'id'=>$sid];});
route('PUT','/api/school-service/schools/{id}',function($p,$b,$u){_ssv_need($u,'edit');_ssv_tables();$id=(int)$p['id'];if(!Db::one("SELECT id FROM school_service_schools WHERE id=?",[$id]))Http::error('مدرسه یافت نشد',404);$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام مدرسه الزامی است',422);$g=in_array($b['gender']??'',['دخترانه','پسرانه'],true)?$b['gender']:'نامشخص';[$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);Db::run("UPDATE school_service_schools SET code=?,name=?,educational_district=?,gender=?,address=?,latitude=?,longitude=? WHERE id=?",[_ssv_norm($b['code']??'')?:null,$name,_ssv_norm($b['educational_district']??'')?:null,$g,_ssv_norm($b['address']??'')?:null,$lat,$lng,$id]);if(isset($b['company_id'])){Db::run("DELETE FROM school_service_school_companies WHERE school_id=?",[$id]);$cid=(int)$b['company_id'];if($cid){if(!Db::one("SELECT id FROM school_service_companies WHERE id=? AND is_active=1",[$cid]))Http::error('شرکت مجری نامعتبر است',422);Db::run("INSERT INTO school_service_school_companies(school_id,company_id,is_primary) VALUES(?,?,1)",[$id,$cid]);}}return ['ok'=>true];});
