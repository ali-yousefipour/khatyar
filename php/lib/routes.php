...
const SITE_VERSION = '1.5.0';   // نسخهٔ سایت — همیشه با نسخهٔ اپ و نام فایل پروژه یکی است
const APP_VERSION = '1.5.0'; // نسخهٔ اپ اندروید — با هر تغییر افزایش می‌یابد
...
function _ssv_plate($b){
  ...
  if($r!==''&&strlen($r)!==2)Http::error('کد منطقه پلاک باید دقیقاً ۲ رقم باشد',422);
  if($l!==''&&mb_strlen($l,'UTF-8')!==1)Http::error('حرف پلاک باید یک حرف باشد',422);
  return [$a?:null,$l?:null,$c?:null,$r?:null];
}
...
function _ssv_sheet_records($rows){
 ...
 $idx=['code'=>$find(['کد مدرسه','code'],1),'school'=>$find(['نام مدرسه','مدرسه','school'],2),'district'=>$find(['ناحیه','ناحیه آموزشی','district'],3),'company'=>$find(['شرکت سرویس‌دهنده','شرکت مجری سرویس دانش‌آموزی','شرکت ارائه دهنده سرویس مدارس','شرکت','company'],4),'gender'=>$find(['جنسیت','gender'],5),'shift'=>$find(['شیفت','shift'],6),'education_level'=>$find(['مقطع تحصیلی','مقطع','education_level'],7),'school_type'=>$find(['نوع مدرسه','school_type'],8),'activity_start'=>$find(['شروع فعالیت','activity_start'],9),'activity_end'=>$find(['پایان فعالیت','activity_end'],10),'morning_start'=>$find(['شروع شیفت صبح','شروع صبح','morning_start'],11),'morning_end'=>$find(['پایان شیفت صبح','پایان صبح','morning_end'],12),'afternoon_start'=>$find(['شروع شیفت عصر','شروع عصر','afternoon_start'],13),'afternoon_end'=>$find(['پایان شیفت عصر','پایان عصر','afternoon_end'],14),'driver_count'=>$find(['تعداد رانندگان','تعداد راننده','driver_count'],15),'student_count'=>$find(['تعداد دانش‌آموز','تعداد دانش آموز','student_count'],16),'address'=>$find(['آدرس','address'],17),'phone'=>$find(['تلفن','تلفن مدرسه','phone'],18),'latitude'=>$find(['عرض جغرافیایی','عرض جغرافیائی','latitude','lat'],19),'longitude'=>$find(['طول جغرافیایی','طول جغرافیائی','longitude','lng'],20),'status'=>$find(['وضعیت','status'],21),'location_registered_at'=>$find(['تاریخ ثبت موقعیت','location_registered_at'],22)];
 $out=[];for($r=1;$r<count($rows);$r++){$x=$rows[$r]??[];$get=function($k)use($x,$idx){return _ssv_excel_scalar($x[$idx[$k]]??'');};$school=_ssv_norm($get('school'));$company=_ssv_norm($get('company'));if($school===''&&$company==='')continue;$gender=_ssv_norm($get('gender'));if(!in_array($gender,['دخترانه','پسرانه','دخترانه-پسرانه','نامشخص'],true))$gender=$gender?:'نامشخص';$out[]=['code'=>_ssv_norm($get('code')),'school'=>$school,'district'=>_ssv_norm($get('district')),'company'=>$company,'gender'=>$gender,'shift'=>_ssv_norm($get('shift')),'education_level'=>_ssv_norm($get('education_level')),'school_type'=>_ssv_norm($get('school_type')),'activity_start'=>_ssv_norm($get('activity_start')),'activity_end'=>_ssv_norm($get('activity_end')),'morning_start'=>_ssv_norm($get('morning_start')),'morning_end'=>_ssv_norm($get('morning_end')),'afternoon_start'=>_ssv_norm($get('afternoon_start')),'afternoon_end'=>_ssv_norm($get('afternoon_end')),'driver_count'=>max(0,(int)$get('driver_count')),'student_count'=>max(0,(int)$get('student_count')),'address'=>_ssv_norm($get('address')),'phone'=>trim($get('phone')),'latitude'=>trim($get('latitude')),'longitude'=>trim($get('longitude')),'status'=>_ssv_norm($get('status'))?:'ثبت‌شده','location_registered_at'=>_ssv_norm($get('location_registered_at'))];}return $out;
}

route('POST','/api/school-service/companies',function($p,$b,$u){_ssv_need($u,'create');_ssv_tables();$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام شرکت الزامی است',422);if(Db::one("SELECT id FROM school_service_companies WHERE name=?",[$name]))Http::error('این شرکت قبلاً ثبت شده است',409);$mobile=trim((string)($b['ceo_mobile']??($b['phone']??'')));$landline=trim((string)($b['landline_phone']??''));[$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);Db::run("INSERT INTO school_service_companies(name,manager_name,phone,ceo_mobile,landline_phone,address,latitude,longitude,declared_school_count,registered_school_count,representative_count,profile_completed,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",[$name,_ssv_norm($b['manager_name']??'')?:null,$mobile?:null,$mobile?:null,$landline?:null,_ssv_norm($b['address']??'')?:null,($b['latitude']??'')!==''?(float)$b['latitude']:null,($b['longitude']??'')!==''?(float)$b['longitude']:null,max(0,(int)($b['declared_school_count']??0)),max(0,(int)($b['registered_school_count']??0)),max(0,(int)($b['representative_count']??0)),!empty($b['profile_completed'])?1:0,isset($b['is_active'])?(int)!!$b['is_active']:1]);return ['ok'=>true,'id'=>(int)Db::pdo()->lastInsertId()];});
route('PUT','/api/school-service/companies/{id}',function($p,$b,$u){_ssv_need($u,'edit');_ssv_tables();$id=(int)$p['id'];if(!Db::one("SELECT id FROM school_service_companies WHERE id=?",[$id]))Http::error('شرکت یافت نشد',404);$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام شرکت الزامی است',422);if(Db::one("SELECT id FROM school_service_companies WHERE name=? AND id<>? LIMIT 1",[$name,$id]))Http::error('نام شرکت دیگری قبلاً با این عنوان ثبت شده است',409);[$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);$mobile=trim((string)($b['ceo_mobile']??($b['phone']??'')));$landline=trim((string)($b['landline_phone']??''));Db::run("UPDATE school_service_companies SET name=?,manager_name=?,phone=?,ceo_mobile=?,landline_phone=?,address=?,latitude=?,longitude=?,declared_school_count=?,registered_school_count=?,representative_count=?,profile_completed=?,is_active=? WHERE id=?",[$name,_ssv_norm($b['manager_name']??'')?:null,$mobile?:null,$mobile?:null,$landline?:null,_ssv_norm($b['address']??'')?:null,$lat,$lng,max(0,(int)($b['declared_school_count']??0)),max(0,(int)($b['registered_school_count']??0)),max(0,(int)($b['representative_count']??0)),!empty($b['profile_completed'])?1:0,isset($b['is_active'])?(int)!!$b['is_active']:1,$id]);return ['ok'=>true];});
route('DELETE','/api/school-service/companies/{id}',function($p,$b,$u){_ssv_need($u,'delete');_ssv_tables();$id=(int)$p['id'];if(!Db::one("SELECT id FROM school_service_companies WHERE id=?",[$id]))Http::error('شرکت یافت نشد',404);Db::run("UPDATE school_service_companies SET is_active=0 WHERE id=?",[$id]);return ['ok'=>true];});
route('POST','/api/school-service/schools',function($p,$b,$u){_ssv_need($u,'create');_ssv_tables();$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام مدرسه الزامی است',422);$code=_ssv_norm($b['code']??'');$district=_ssv_norm($b['educational_district']??'');if($district!==''&&!_ssv_valid_option('school_service_districts',$district))Http::error('ناحیه آموزشی انتخاب‌شده معتبر یا فعال نیست.',422);if($code!==''&&Db::one("SELECT id FROM school_service_schools WHERE code=? LIMIT 1",[$code]))Http::error('کد مدرسه قبلاً ثبت شده است.',409);$g=in_array($b['gender']??'',['دخترانه','پسرانه','دخترانه-پسرانه'],true)?$b['gender']:'نامشخص';$cid=(int)($b['company_id']??0);_ssv_school_company_validate(0,$cid,$district);[$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);$vals=[$code?:null,$name,$district?:null,$g,_ssv_norm($b['shift']??'')?:null,_ssv_norm($b['education_level']??'')?:null,_ssv_norm($b['school_type']??'')?:null,_ssv_norm($b['activity_start']??'')?:null,_ssv_norm($b['activity_end']??'')?:null,_ssv_norm($b['morning_start']??'')?:null,_ssv_norm($b['morning_end']??'')?:null,_ssv_norm($b['afternoon_start']??'')?:null,_ssv_norm($b['afternoon_end']??'')?:null,max(0,(int)($b['driver_count']??0)),max(0,(int)($b['student_count']??0)),_ssv_norm($b['address']??'')?:null,trim((string)($b['phone']??''))?:null,$lat,$lng,_ssv_norm($b['status']??'ثبت‌شده')?:'ثبت‌شده',_ssv_norm($b['location_registered_at']??'')?:null];$pdo=Db::pdo();$pdo->beginTransaction();try{Db::run("INSERT INTO school_service_schools(code,name,educational_district,gender,shift,education_level,school_type,activity_start,activity_end,morning_start,morning_end,afternoon_start,afternoon_end,driver_count,student_count,address,phone,latitude,longitude,status,location_registered_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",$vals);$id=(int)$pdo->lastInsertId();if($cid>0)Db::run("INSERT INTO school_service_school_companies(school_id,company_id,is_primary) VALUES(?,?,1)",[$id,$cid]);$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}return ['ok'=>true,'id'=>$id];});
route('PUT','/api/school-service/schools/{id}',function($p,$b,$u){_ssv_need($u,'edit');_ssv_tables();$id=(int)$p['id'];if(!Db::one("SELECT id FROM school_service_schools WHERE id=? AND is_active=1",[$id]))Http::error('مدرسه یافت نشد',404);$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام مدرسه الزامی است',422);$code=_ssv_norm($b['code']??'');$district=_ssv_norm($b['educational_district']??'');if($district!==''&&!_ssv_valid_option('school_service_districts',$district))Http::error('ناحیه آموزشی انتخاب‌شده معتبر یا فعال نیست.',422);if($code!==''&&Db::one("SELECT id FROM school_service_schools WHERE code=? AND id<>? LIMIT 1",[$code,$id]))Http::error('کد مدرسه قبلاً برای مدرسه دیگری ثبت شده است.',409);$g=in_array($b['gender']??'',['دخترانه','پسرانه','دخترانه-پسرانه'],true)?$b['gender']:'نامشخص';$cid=array_key_exists('company_id',$b)?(int)($b['company_id']??0):null;_ssv_school_company_validate(0,$cid??0,$district);[$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);$pdo=Db::pdo();$pdo->beginTransaction();try{Db::run("UPDATE school_service_schools SET code=?,name=?,educational_district=?,gender=?,shift=?,education_level=?,school_type=?,activity_start=?,activity_end=?,morning_start=?,morning_end=?,afternoon_start=?,afternoon_end=?,driver_count=?,student_count=?,address=?,phone=?,latitude=?,longitude=?,status=?,location_registered_at=? WHERE id=?",[$code?:null,$name,$district?:null,$g,_ssv_norm($b['shift']??'')?:null,_ssv_norm($b['education_level']??'')?:null,_ssv_norm($b['school_type']??'')?:null,_ssv_norm($b['activity_start']??'')?:null,_ssv_norm($b['activity_end']??'')?:null,_ssv_norm($b['morning_start']??'')?:null,_ssv_norm($b['morning_end']??'')?:null,_ssv_norm($b['afternoon_start']??'')?:null,_ssv_norm($b['afternoon_end']??'')?:null,max(0,(int)($b['driver_count']??0)),max(0,(int)($b['student_count']??0)),_ssv_norm($b['address']??'')?:null,trim((string)($b['phone']??''))?:null,$lat,$lng,_ssv_norm($b['status']??'ثبت‌شده')?:'ثبت‌شده',_ssv_norm($b['location_registered_at']??'')?:null,$id]);if(array_key_exists('company_id',$b)){Db::run("DELETE FROM school_service_school_companies WHERE school_id=?",[$id]);if($cid>0)Db::run("INSERT INTO school_service_school_companies(school_id,company_id,is_primary) VALUES(?,?,1)",[$id,$cid]);}$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}return ['ok'=>true];});
route('DELETE','/api/school-service/schools/{id}',function($p,$b,$u){_ssv_need($u,'delete');_ssv_tables();$id=(int)$p['id'];if(!Db::one("SELECT id FROM school_service_schools WHERE id=?",[$id]))Http::error('مدرسه یافت نشد',404);Db::run("UPDATE school_service_schools SET is_active=0 WHERE id=?",[$id]);return ['ok'=>true];});
...
route('GET','/api/school-service/meta',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();$districts=array_column(Db::all("SELECT title FROM school_service_districts WHERE is_active=1 ORDER BY sort_order,id"),'title');$vehicleTypes=array_column(Db::all("SELECT title FROM school_service_vehicle_types WHERE is_active=1 ORDER BY sort_order,id"),'title');$vehicleColors=array_column(Db::all("SELECT title FROM school_service_vehicle_colors WHERE is_active=1 ORDER BY sort_order,id"),'title');$violations=Db::all("SELECT id,title FROM school_service_violation_types WHERE is_active=1 ORDER BY sort_order,id");return ['districts'=>$districts?:['۱','۲','۳','۴','۵','۶','۷','تبادکان'],'genders'=>['دخترانه','پسرانه','دخترانه-پسرانه','نامشخص'],'driver_genders'=>['خانم','آقا','نامشخص'],'vehicle_types'=>$vehicleTypes?:['سمند','سورن','پژو','پراید','تیبا','دنا','رانا','اطلس','کوییک','سایر'],'vehicle_colors'=>$vehicleColors?:['سفید','زرد','مشکی','نقره‌ای','خاکستری','آبی','قرمز','سبز','سایر'],'violations'=>$violations];});
route('GET','/api/school-service/companies',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();$q=_ssv_norm($_GET['search']??'');$limit=min(200,max(1,(int)($_GET['limit']??100)));$includeInactive=!empty($_GET['include_inactive']);$w=[];$args=[];if(!$includeInactive)$w[]='c.is_active=1';if($q!==''){$w[]='c.name LIKE ?';$args[]='%'.$q.'%';}$where=$w?'WHERE '.implode(' AND ',$w):'';return ['items'=>Db::all("SELECT c.id,c.name,c.manager_name,c.phone,c.ceo_mobile,c.landline_phone,c.address,c.latitude,c.longitude,c.declared_school_count,c.registered_school_count,c.representative_count,c.profile_completed,c.is_active,COUNT(DISTINCT sc.school_id) actual_school_count,COUNT(DISTINCT sc.school_id) school_count FROM school_service_companies c LEFT JOIN school_service_school_companies sc ON sc.company_id=c.id $where GROUP BY c.id ORDER BY c.name LIMIT $limit",$args)];});
route('GET','/api/school-service/schools',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();$q=_ssv_norm($_GET['search']??'');$company=(int)($_GET['company_id']??0);$district=_ssv_norm($_GET['district']??'');$c=['s.is_active=1'];$args=[];if($q!==''){$c[]='(s.name LIKE ? OR s.code LIKE ? OR c.name LIKE ?)';$args[]='%'.$q.'%';$args[]='%'.$q.'%';$args[]='%'.$q.'%';}if($company){$c[]='EXISTS(SELECT 1 FROM school_service_school_companies x WHERE x.school_id=s.id AND x.company_id=?)';$args[]=$company;}if($district!==''){$c[]='(s.educational_district=? OR s.educational_district IS NULL OR s.educational_district=\'\')';$args[]=$district;}$where=implode(' AND ',$c);return ['items'=>Db::all("SELECT s.id,s.code,s.name,s.educational_district,s.gender,s.shift,s.education_level,s.school_type,s.activity_start,s.activity_end,s.morning_start,s.morning_end,s.afternoon_start,s.afternoon_end,s.driver_count,s.student_count,s.address,s.phone,s.latitude,s.longitude,s.status,s.location_registered_at,COALESCE(MAX(CASE WHEN sc.is_primary=1 THEN sc.company_id END),MIN(sc.company_id)) company_id,GROUP_CONCAT(DISTINCT c.name ORDER BY c.name SEPARATOR ', ') company_names FROM school_service_schools s LEFT JOIN school_service_school_companies sc ON sc.school_id=s.id LEFT JOIN school_service_companies c ON c.id=sc.company_id WHERE $where GROUP BY s.id ORDER BY s.name LIMIT 500",$args)];});
...
route('POST','/api/school-service/inspections',function($p,$b,$u){
  ...
  $clientUuid=_ssv_norm($b['client_uuid']??($_SERVER['HTTP_X_CLIENT_UUID']??''));
  if($clientUuid!==''){
    $existing=Db::one("SELECT id FROM school_service_inspections WHERE client_uuid=?",[$clientUuid]);
    if($existing)return ['ok'=>true,'id'=>(int)$existing['id'],'duplicate'=>true];
  }
  [$a,$l,$c,$r]=_ssv_plate($b);
  $schoolId=(int)($b['school_id']??0);$companyId=(int)($b['company_id']??0);
  if(!$schoolId)$schoolId=null;if(!$companyId)$companyId=null;
  $district=_ssv_norm($b['educational_district']??'');
  if($district!==''&&!_ssv_valid_option('school_service_districts',$district))Http::error('ناحیه آموزشی انتخاب‌شده معتبر یا فعال نیست.',422);
  $vehicleTypeChoice=_ssv_norm($b['vehicle_type']??'');$vehicleTypeOther=_ssv_norm($b['vehicle_type_other']??'');
  $vehicleType=$vehicleTypeChoice==='سایر'?$vehicleTypeOther:$vehicleTypeChoice;
  if($vehicleTypeChoice!==''&&$vehicleTypeChoice!=='سایر'&&!_ssv_valid_option('school_service_vehicle_types',$vehicleTypeChoice))Http::error('نوع خودرو انتخاب‌شده معتبر یا فعال نیست.',422);
  if($vehicleTypeChoice==='سایر'&&$vehicleType==='')Http::error('نوع خودرو را وارد کنید.',422);
  $vehicleColorChoice=_ssv_norm($b['vehicle_color']??'');$vehicleColorOther=_ssv_norm($b['vehicle_color_other']??'');
  $vehicleColor=$vehicleColorChoice==='سایر'?$vehicleColorOther:$vehicleColorChoice;
  if($vehicleColorChoice!==''&&$vehicleColorChoice!=='سایر'&&!_ssv_valid_option('school_service_vehicle_colors',$vehicleColorChoice))Http::error('رنگ خودرو انتخاب‌شده معتبر یا فعال نیست.',422);
  if($vehicleColorChoice==='سایر'&&$vehicleColor==='')Http::error('رنگ خودرو را وارد کنید.',422);
  _ssv_school_company_validate($schoolId??0,$companyId??0,$district);
  $viol=$b['violation_ids']??[];if(is_string($viol))$viol=json_decode($viol,true)?:[];$viol=array_values(array_unique(array_map('intval',(array)$viol)));
  $gender=in_array($b['school_gender']??'',['دخترانه','پسرانه','دخترانه-پسرانه','نامشخص'],true)?$b['school_gender']:'نامشخص';
  $dg=in_array($b['driver_gender']??'',['خانم','آقا','نامشخص'],true)?$b['driver_gender']:'نامشخص';
  $cert=in_array($b['certificate_status']??'',['معتبر','نامعتبر','ارائه نشد'],true)?$b['certificate_status']:'ارائه نشد';
  $front=max(0,(int)($b['passenger_front_count']??0));$rear=max(0,(int)($b['passenger_rear_count']??0));$pass=$front+$rear;
  if(!isset($b['passenger_front_count'])&&!isset($b['passenger_rear_count']))$pass=max(0,(int)($b['passenger_count']??0));
  $date=_ssv_en($b['violation_date']??'');$time=trim((string)($b['violation_time']??''));
  if($date!==''&&!preg_match('/^14\d{2}[\/\-]\d{1,2}[\/\-]\d{1,2}$/',$date))Http::error('تاریخ ثبت تخلف نامعتبر است.',422);
  if($time!==''&&!preg_match('/^(?:[01]?\d|2[0-3]):[0-5]\d$/',$time))Http::error('ساعت ثبت تخلف نامعتبر است.',422);
  [$lat,$lng]=validGeo($b['latitude']??null,$b['longitude']??null);
  Db::run("INSERT INTO school_service_inspections(inspector_user_id,client_uuid,educational_district,company_id,school_id,school_gender,plate_three,plate_letter,plate_two,plate_region,iran_code,vehicle_type,vehicle_color,passenger_front_count,passenger_rear_count,passenger_count,driver_gender,certificate_status,violation_date,violation_time,location_text,latitude,longitude,description) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    [(int)$u['id'],$clientUuid?:null,$district,$companyId,$schoolId,$gender,$a,$l,$c,$r,'ایران',$vehicleType?:null,$vehicleColor?:null,$front,$rear,$pass,$dg,$cert,$date?:null,$time?:null,_ssv_norm($b['location_text']??'')?:null,$lat,$lng,trim((string)($b['description']??''))?:null]);
  $id=(int)Db::pdo()->lastInsertId();
  foreach($viol as $vid)if(Db::one("SELECT id FROM school_service_violation_types WHERE id=? AND is_active=1",[$vid]))Db::run("INSERT IGNORE INTO school_service_inspection_violations(inspection_id,violation_type_id) VALUES(?,?)",[$id,$vid]);
  return ['ok'=>true,'id'=>$id];
});
// Excel import routes are implemented in school_service_routes_extra.php.
route('GET','/api/school-service/export',function($p,$b,$u){_ssv_need($u,'report');_ssv_tables();$type=$_GET['type']??'inspections';$rows=[];$head=[];if($type==='schools'){$head=['ردیف','کد مدرسه','نام مدرسه','ناحیه','شرکت سرویس‌دهنده','جنسیت','شیفت','مقطع تحصیلی','نوع مدرسه','شروع فعالیت','پایان فعالیت','شروع شیفت صبح','پایان شیفت صبح','شروع شیفت عصر','پایان شیفت عصر','تعداد رانندگان','تعداد دانش‌آموز','آدرس','تلفن','عرض جغرافیایی','طول جغرافیایی','وضعیت','تاریخ ثبت موقعیت'];$rows=Db::all("SELECT s.*,GROUP_CONCAT(DISTINCT c.name ORDER BY c.name SEPARATOR '، ') company_names FROM school_service_schools s LEFT JOIN school_service_school_companies sc ON sc.school_id=s.id LEFT JOIN school_service_companies c ON c.id=sc.company_id WHERE s.is_active=1 GROUP BY s.id ORDER BY s.name LIMIT 10000");foreach($rows as $i=>&$r)$r=[$i+1,$r['code'],$r['name'],$r['educational_district'],$r['company_names'],$r['gender'],$r['shift'],$r['education_level'],$r['school_type'],$r['activity_start'],$r['activity_end'],$r['morning_start'],$r['morning_end'],$r['afternoon_start'],$r['afternoon_end'],$r['driver_count'],$r['student_count'],$r['address'],$r['phone'],$r['latitude'],$r['longitude'],$r['status'],$r['location_registered_at']];unset($r);}elseif($type==='companies'){$head=['ردیف','شناسه','نام شرکت','نام مدیرعامل','تلفن همراه مدیرعامل','تلفن ثابت','آدرس','عرض جغرافیایی','طول جغرافیایی','تعداد مدارس','مدارس ثبت‌شده','تعداد نمایندگان','وضعیت','تکمیل پروفایل'];$rows=Db::all("SELECT c.*,COUNT(DISTINCT sc.school_id) actual_school_count FROM school_service_companies c LEFT JOIN school_service_school_companies sc ON sc.company_id=c.id WHERE c.is_active=1 GROUP BY c.id ORDER BY c.name LIMIT 5000");foreach($rows as $i=>&$r)$r=[$i+1,$r['id'],$r['name'],$r['manager_name'],$r['ceo_mobile']??$r['phone'],$r['landline_phone'],$r['address'],$r['latitude'],$r['longitude'],$r['declared_school_count'],$r['registered_school_count'],$r['representative_count'],'فعال',$r['profile_completed']?'تکمیل‌شده':'ناقص'];unset($r);}else{$head=['ردیف','ناحیه','شرکت','نوع مدرسه','مدرسه','پلاک','نوع خودرو','رنگ خودرو','تخلفات','سرنشین جلو','سرنشین عقب','مجموع سرنشینان','نوع راننده','وضعیت گواهی صلاحیت','تاریخ','ساعت','محل ثبت تخلف','توضیحات','بازرس'];$rows=Db::all("SELECT i.*,s.name school_name,c.name company_name,TRIM(CONCAT(COALESCE(u.first_name,''),' ',COALESCE(u.last_name,''))) inspector_name FROM school_service_inspections i LEFT JOIN school_service_schools s ON s.id=i.school_id LEFT JOIN school_service_companies c ON c.id=i.company_id LEFT JOIN users u ON u.id=i.inspector_user_id ORDER BY i.id DESC LIMIT 20000");foreach($rows as $r){$vs=Db::all("SELECT v.title FROM school_service_inspection_violations x JOIN school_service_violation_types v ON v.id=x.violation_type_id WHERE x.inspection_id=?",[(int)$r['id']]);$rows2[]=[count($rows2??[])+1,$r['educational_district'],$r['company_name'],$r['school_gender'],$r['school_name'],trim(($r['plate_two']??'').' '.($r['plate_letter']??'').' '.($r['plate_three']??'').' ایران '.($r['plate_region']??'')),$r['vehicle_type'],$r['vehicle_color'],implode('، ',array_column($vs,'title')),$r['passenger_front_count'],$r['passenger_rear_count'],$r['passenger_count'],$r['driver_gender'],$r['certificate_status'],$r['violation_date'],$r['violation_time'],$r['location_text'],$r['description'],$r['inspector_name']];}$rows=$rows2??[];}$xw=new XlsxWriter($head);foreach($rows as $row)$xw->addRow($row);$xw->output('سرویس_مدارس_'.$type.'.xlsx','سرویس مدارس');},false,99);


require_once __DIR__.'/school_service_routes_extra.php';
route('GET','/api/school-service/permissions',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();if((int)($u['level']??0)>0 && (int)($u['level']??0)>3 && empty($u['is_admin']))Http::error('فقط سطوح مدیریتی مجاز می‌توانند دسترسی‌ها را مدیریت کنند.',403);$rows=Db::all("SELECT r.id,r.title,r.level,COALESCE(p.can_view,0) can_view,COALESCE(p.can_create,0) can_create,COALESCE(p.can_edit,0) can_edit,COALESCE(p.can_delete,0) can_delete,COALESCE(p.can_import,0) can_import,COALESCE(p.can_report,0) can_report FROM roles r LEFT JOIN school_service_permissions p ON p.role_id=r.id ORDER BY r.level,r.title");return ['items'=>$rows];});
route('POST','/api/school-service/permissions/{role_id}',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();if((int)($u['level']??0)>0 && (int)($u['level']??0)>3 && empty($u['is_admin']))Http::error('فقط سطوح مدیریتی مجاز می‌توانند دسترسی‌ها را مدیریت کنند.',403);$rid=(int)$p['role_id'];if(!Db::one("SELECT id FROM roles WHERE id=?",[$rid]))Http::error('سمت نامعتبر است',422);$vals=[];foreach(['view','create','edit','delete','import','report'] as $k)$vals[$k]=!empty($b['can_'.$k])?1:0;Db::run("INSERT INTO school_service_permissions(role_id,can_view,can_create,can_edit,can_delete,can_import,can_report) VALUES(?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE can_view=VALUES(can_view),can_create=VALUES(can_create),can_edit=VALUES(can_edit),can_delete=VALUES(can_delete),can_import=VALUES(can_import),can_report=VALUES(can_report)",[$rid,$vals['view'],$vals['create'],$vals['edit'],$vals['delete'],$vals['import'],$vals['report']]);return ['ok'=>true];},false,99);
