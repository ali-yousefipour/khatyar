...
route('POST','/api/school-service/companies',function($p,$b,$u){_ssv_need($u,'edit');_ssv_tables();$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام شرکت الزامی است',422);if(Db::one("SELECT id FROM school_service_companies WHERE name=?",[$name]))Http::error('این شرکت قبلاً ثبت شده است',409);Db::run("INSERT INTO school_service_companies(name,manager_name,phone,address,manager_mobile,landline_phone,latitude,longitude,registered_school_count,representative_count,status,profile_complete) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",[$name,_ssv_norm($b['manager_name']??'')?:null,trim($b['phone']??'')?:null,_ssv_norm($b['address']??'')?:null,trim($b['manager_mobile']??'')?:null,trim($b['landline_phone']??'')?:null,$b['latitude']!==''&&isset($b['latitude'])?(float)$b['latitude']:null,$b['longitude']!==''&&isset($b['longitude'])?(float)$b['longitude']:null,max(0,(int)($b['registered_school_count']??0)),max(0,(int)($b['representative_count']??0)),_ssv_norm($b['status']??'فعال')?:'فعال',!empty($b['profile_complete'])?1:0]);return ['ok'=>true,'id'=>(int)Db::pdo()->lastInsertId()];});
