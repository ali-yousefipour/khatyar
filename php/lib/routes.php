...
route('POST','/api/school-service/schools',function($p,$b,$u){_ssv_need($u,'create');_ssv_tables();$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام مدرسه الزامی است',422);$g=in_array($b['gender']??'',['دخترانه','پسرانه','دخترانه-پسرانه'],true)?$b['gender']:'نامشخص';$vals=[_ssv_norm($b['code']??'')?:null,$name,_ssv_norm($b['educational_district']??'')?:null,$g,_ssv_norm($b['shift']??'')?:null,_ssv_norm($b['education_level']??'')?:null,_ssv_norm($b['school_type']??'')?:null,_ssv_norm($b['activity_start']??'')?:null,_ssv_norm($b['activity_end']??'')?:null,_ssv_norm($b['morning_start']??'')?:null,_ssv_norm($b['morning_end']??'')?:null,_ssv_norm($b['afternoon_start']??'')?:null,_ssv_norm($b['afternoon_end']??'')?:null,max(0,(int)($b['driver_count']??0)),max(0,(int)($b['student_count']??0)),_ssv_norm($b['address']??'')?:null,trim((string)($b['phone']??''))?:null,($b['latitude']??'')!==''?(float)$b['latitude']:null,($b['longitude']??'')!==''?(float)$b['longitude']:null,_ssv_norm($b['status']??'ثبت‌شده')?:'ثبت‌شده',_ssv_norm($b['location_registered_at']??'')?:null];Db::run("INSERT INTO school_service_schools(code,name,educational_district,gender,shift,education_level,school_type,activity_start,activity_end,morning_start,morning_end,afternoon_start,afternoon_end,driver_count,student_count,address,phone,latitude,longitude,status,location_registered_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",$vals);$id=(int)Db::pdo()->lastInsertId();if(isset($b['company_id'])&&(int)$b['company_id']>0)Db::run("INSERT INTO school_service_school_companies(school_id,company_id,is_primary) VALUES(?,?,1)",[$id,(int)$b['company_id']]);return ['ok'=>true,'id'=>$id];});
...
route('GET','/api/school-service/export',function($p,$b,$u){_ssv_need($u,'report');_ssv_tables();$type=$_GET['type']??'inspections';$rows=[];$head=[];if($type==='schools'){$head=['ردیف','کد مدرسه','نام مدرسه','ناحیه آموزشی','نوع مدرسه','شرکت/شرکت‌های مجری','آدرس'];$rows=Db::all("SELECT s.id,s.code,s.name,s.educational_district,s.gender,s.address,GROUP_CONCAT(DISTINCT c.name ORDER BY c.name SEPARATOR '، ') company_names FROM school_service_schools s LEFT JOIN school_service_school_companies sc ON sc.school_id=s.id LEFT JOIN school_service_companies c ON c.id=sc.company_id WHERE s.is_active=1 GROUP BY s.id ORDER BY s.name LIMIT 10000");foreach($rows as $i=>&$r)$r=[$i+1,$r['code'],$r['name'],$r['educational_district'],$r['gender'],$r['company_names'],$r['address']];unset($r);}elseif($type==='companies'){$head=['ردیف','شناسه','نام شرکت','نام مدیرعامل','تلفن همراه مدیرعامل','تلفن ثابت','آدرس','عرض جغرافیایی','طول جغرافیایی','تعداد مدارس','مدارس ثبت‌شده','تعداد نمایندگان','وضعیت','تکمیل پروفایل'];$rows=Db::all("SELECT c.*,COUNT(DISTINCT sc.school_id) actual_school_count FROM school_service_companies c LEFT JOIN school_service_school_companies sc ON sc.company_id=c.id WHERE c.is_active=1 GROUP BY c.id ORDER BY c.name LIMIT 5000");foreach($rows as $i=>&$r)$r=[$i+1,$r['id'],$r['name'],$r['manager_name'],$r['ceo_mobile']??$r['phone'],$r['landline_phone'],$r['address'],$r['latitude'],$r['longitude'],$r['declared_school_count'],$r['registered_school_count'],$r['representative_count'],'فعال',$r['profile_completed']?'تکمیل‌شده':'ناقص'];unset($r);}else{$head=['ردیف','ناحیه','شرکت','نوع مدرسه','مدرسه','پلاک','نوع خودرو','رنگ خودرو','تخلفات','تعداد سرنشین بدون راننده','نوع راننده','وضعیت گواهی صلاحیت','تاریخ','ساعت','محل ثبت تخلف','توضیحات','بازرس'];$rows=Db::all("SELECT i.*,s.name school_name,c.name company_name,TRIM(CONCAT(COALESCE(u.first_name,''),' ',COALESCE(u.last_name,''))) inspector_name FROM school_service_inspections i LEFT JOIN school_service_schools s ON s.id=i.school_id LEFT JOIN school_service_companies c ON c.id=i.company_id LEFT JOIN users u ON u.id=i.inspector_user_id ORDER BY i.id DESC LIMIT 20000");foreach($rows as $r){$vs=Db::all("SELECT v.title FROM school_service_inspection_violations x JOIN school_service_violation_types v ON v.id=x.violation_type_id WHERE x.inspection_id=?",[(int)$r['id']]);$rows2[]=[count($rows2??[])+1,$r['educational_district'],$r['company_name'],$r['school_gender'],$r['school_name'],trim(($r['plate_two']??'').' '.($r['plate_letter']??'').' '.($r['plate_three']??'').' ایران '.($r['plate_region']??'')),$r['vehicle_type'],$r['vehicle_color'],implode('، ',array_column($vs,'title')),$r['passenger_count'],$r['driver_gender'],$r['certificate_status'],$r['violation_date'],$r['violation_time'],$r['location_text'],$r['description'],$r['inspector_name']];}$rows=$rows2??[];}$xw=new XlsxWriter($head);foreach($rows as $row)$xw->addRow($row);$xw->output('سرویس_مدارس_'.$type.'.xlsx','سرویس مدارس');},false,99);
route('GET','/api/school-service/permissions',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();if((int)($u['level']??0)>0 && (int)($u['level']??0)>3 && empty($u['is_admin']))Http::error('فقط سطوح مدیریتی مجاز می‌توانند دسترسی‌ها را مدیریت کنند.',403);$rows=Db::all("SELECT r.id,r.title,r.level,COALESCE(p.can_view,0) can_view,COALESCE(p.can_create,0) can_create,COALESCE(p.can_edit,0) can_edit,COALESCE(p.can_delete,0) can_delete,COALESCE(p.can_import,0) can_import,COALESCE(p.can_report,0) can_report FROM roles r LEFT JOIN school_service_permissions p ON p.role_id=r.id ORDER BY r.level,r.title");return ['items'=>$rows];});
route('POST','/api/school-service/permissions/{role_id}',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();if((int)($u['level']??0)>0 && (int)($u['level']??0)>3 && empty($u['is_admin']))Http::error('فقط سطوح مدیریتی مجاز می‌توانند دسترسی‌ها را مدیریت کنند.',403);$rid=(int)$p['role_id'];if(!Db::one("SELECT id FROM roles WHERE id=?",[$rid]))Http::error('سمت نامعتبر است',422);$vals=[];foreach(['view','create','edit','delete','import','report'] as $k)$vals[$k]=!empty($b['can_'.$k])?1:0;Db::run("INSERT INTO school_service_permissions(role_id,can_view,can_create,can_edit,can_delete,can_import,can_report) VALUES(?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE can_view=VALUES(can_view),can_create=VALUES(can_create),can_edit=VALUES(can_edit),can_delete=VALUES(can_delete),can_import=VALUES(can_import),can_report=VALUES(can_report)",[$rid,$vals['view'],$vals['create'],$vals['edit'],$vals['delete'],$vals['import'],$vals['report']]);return ['ok'=>true];},false,99);
