...
route('GET','/api/school-service/schools-page',function($p,$b,$u){_ssv_need($u,'view');_ssv_tables();$page=max(1,(int)($_GET['page']??1));$size=max(10,min(100,(int)($_GET['page_size']??25)));$q=_ssv_norm($_GET['search']??'');$district=_ssv_norm($_GET['district']??'');$where=['1=1'];$args=[];if($q!==''){$where[]='(s.name LIKE ? OR s.code LIKE ?)';$args[]='%'.$q.'%';$args[]='%'.$q.'%';}if($district!==''){$where[]='s.educational_district=?';$args[]=$district;}$w=implode(' AND ',$where);$total=(int)(Db::one("SELECT COUNT(*) n FROM school_service_schools s WHERE $w",$args)['n']??0);$rows=Db::all("SELECT s.id,s.code,s.name,s.educational_district,s.gender,s.address,s.is_active,GROUP_CONCAT(DISTINCT c.name ORDER BY c.name SEPARATOR ', ') company_names FROM school_service_schools s LEFT JOIN school_service_school_companies sc ON sc.school_id=s.id LEFT JOIN school_service_companies c ON c.id=sc.company_id WHERE $w GROUP BY s.id ORDER BY s.name LIMIT $size OFFSET ".(($page-1)*$size),$args);return ['items'=>$rows,'page'=>$page,'page_size'=>$size,'total'=>$total,'pages'=>max(1,(int)ceil($total/$size))];});
