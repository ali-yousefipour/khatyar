...
route('PUT','/api/school-service/schools/{id}',function($p,$b,$u){_ssv_need($u,'edit');_ssv_tables();$id=(int)$p['id'];if(!Db::one("SELECT id FROM school_service_schools WHERE id=?",[$id]))Http::error('مدرسه یافت نشد',404);$name=_ssv_norm($b['name']??'');if($name==='')Http::error('نام مدرسه الزامی است',422);$g=in_array($b['gender']??'',['دخترانه','پسرانه','دخترانه-پسرانه'],true)?$b['gender']:'نامشخص';Db::run("UPDATE school_service_schools SET code=?,name=?,educational_district=?,gender=?,shift=?,education_level=?,school_type=?,activity_start=?,activity_end=?,morning_start=?,morning_end=?,afternoon_start=?,afternoon_end=?,driver_count=?,student_count=?,address=?,phone=?,latitude=?,longitude=?,status=?,location_registered_at=? WHERE id=?",[_ssv_norm($b['code']??'')?:null,$name,_ssv_norm($b['educational_district']??'')?:null,$g,_ssv_norm($b['shift']??'')?:null,_ssv_norm($b['education_level']??'')?:null,_ssv_norm($b['school_type']??'')?:null,_ssv_norm($b['activity_start']??'')?:null,_ssv_norm($b['activity_end']??'')?:null,_ssv_norm($b['morning_start']??'')?:null,_ssv_norm($b['morning_end']??'')?:null,_ssv_norm($b['afternoon_start']??'')?:null,_ssv_norm($b['afternoon_end']??'')?:null,max(0,(int)($b['driver_count']??0)),max(0,(int)($b['student_count']??0)),_ssv_norm($b['address']??'')?:null,trim((string)($b['phone']??''))?:null,($b['latitude']??'')!==''?(float)$b['latitude']:null,($b['longitude']??'')!==''?(float)$b['longitude']:null,_ssv_norm($b['status']??'ثبت‌شده')?:'ثبت‌شده',_ssv_norm($b['location_registered_at']??'')?:null,$id]);if(isset($b['company_id'])){Db::run("DELETE FROM school_service_school_companies WHERE school_id=?",[$id]);$cid=(int)$b['company_id'];if($cid)Db::run("INSERT INTO school_service_school_companies(school_id,company_id,is_primary) VALUES(?,?,1)",[$id,$cid]);}return ['ok'=>true];});
