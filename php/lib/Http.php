...
class Http {
  ...
  private static function beginClientRequest($body) {
    ...
    [$uuid,$method,$path] = $key;
    try {
      Db::run("CREATE TABLE IF NOT EXISTS api_client_requests (
        ...
        PRIMARY KEY(id),
        UNIQUE KEY uq_api_client_request(client_uuid,method,path),
        KEY idx_api_client_request_created(created_at)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
      $row = Db::one("SELECT id,status_code,response_json,created_at FROM api_client_requests WHERE client_uuid=? AND method=? AND path=? LIMIT 1",[$uuid,$method,$path]);
      if ($row) {
        ...
        Db::run("DELETE FROM api_client_requests WHERE id=?",[$row['id']]);
      }
      try { Db::run("INSERT INTO api_client_requests(client_uuid,method,path,status_code) VALUES(?,?,?,102)",[$uuid,$method,$path]); }
      catch (Throwable $e) {
        $row = Db::one("SELECT id,status_code,response_json FROM api_client_requests WHERE client_uuid=? AND method=? AND path=? LIMIT 1",[$uuid,$method,$path]);
        if ($row && (int)$row['status_code'] >= 200 && (int)$row['status_code'] < 300 && !empty($row['response_json'])) {
          $data = json_decode($row['response_json'], true);
          if (json_last_error() === JSON_ERROR_NONE) self::json($data,(int)$row['status_code']);
        }
      }
      self::$clientRequest = [$uuid,$method,$path];
    } catch (Throwable $e) {
      // Idempotency is an additional safety layer; a DB permission/problem must never break the API itself.
      self::$clientRequest = null;
    }
  }
  private static function finishClientRequest($data,$code) {
    if (!self::$clientRequest || !class_exists('Db')) return;
    [$uuid,$method,$path] = self::$clientRequest;
    try {
      if ($code >= 200 && $code < 300) {
        Db::run("UPDATE api_client_requests SET status_code=?,response_json=?,completed_at=NOW() WHERE client_uuid=? AND method=? AND path=?",[$code,json_encode($data,JSON_UNESCAPED_UNICODE),$uuid,$method,$path]);
      } else {
        Db::run("DELETE FROM api_client_requests WHERE client_uuid=? AND method=? AND path=?",[$uuid,$method,$path]);
      }
    } catch (Throwable $e) {}
  }
