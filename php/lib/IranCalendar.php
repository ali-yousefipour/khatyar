...
class IranCalendar {
    ...
    private static function env($n,$d=null){$v=getenv($n);return $v===false||trim((string)$v)===''?$d:trim($v);}
    ...
    public static function monthLength($y,$m){if($m<=6)return 31;if($m<=11)return 30;return self::leap($y)?30:29;}
    ...
    /* فهرست مرجع تعطیلات رسمی کشور؛ برای سال‌های ۱۴۰۴ و ۱۴۰۵ عمداً مستقل از سرویس‌های ثالث نگهداری می‌شود. */
    private static function fallbackEvents($y){
        $sets=[
            1404=>[
                ['01','01','آغاز نوروز'],['01','02','شهادت حضرت علی (ع) و عید نوروز'],['01','03','عید نوروز'],['01','04','عید نوروز'],
                ['01','11','عید سعید فطر'],['01','12','تعطیل به مناسبت عید سعید فطر و روز جمهوری اسلامی ایران'],['01','13','روز طبیعت'],
                ['02','04','شهادت امام جعفر صادق (ع)'],['03','14','رحلت امام خمینی (ره)'],['03','15','قیام ۱۵ خرداد'],['03','16','عید سعید قربان'],['03','24','عید سعید غدیر خم'],
                ['04','14','تاسوعای حسینی'],['04','15','عاشورای حسینی'],['05','23','اربعین حسینی'],['05','31','رحلت حضرت رسول اکرم (ص) و شهادت امام حسن مجتبی (ع)'],
                ['06','02','شهادت امام رضا (ع)'],['06','10','شهادت امام حسن عسکری (ع) و آغاز امامت حضرت ولیعصر (عج)'],['06','19','میلاد حضرت رسول اکرم (ص) و میلاد امام جعفر صادق (ع)'],
                ['09','03','شهادت حضرت فاطمه زهرا (س)'],['10','13','ولادت امام علی (ع) و روز پدر'],['10','27','مبعث حضرت رسول اکرم (ص)'],
                ['11','15','ولادت حضرت قائم (عج)'],['11','22','پیروزی انقلاب اسلامی ایران'],['12','20','شهادت حضرت علی (ع)'],['12','29','روز ملی شدن صنعت نفت ایران']
            ],
            1405=>[
                ['01','01','عید سعید فطر و آغاز نوروز'],['01','02','تعطیل به مناسبت عید سعید فطر و عید نوروز'],['01','03','عید نوروز'],['01','04','عید نوروز'],
                ['01','12','روز جمهوری اسلامی ایران'],['01','13','روز طبیعت'],['01','25','شهادت امام جعفر صادق (ع)'],['03','06','عید سعید قربان'],
                ['03','14','عید سعید غدیر خم و رحلت امام خمینی (ره)'],['03','15','قیام ۱۵ خرداد'],['04','03','تاسوعای حسینی'],['04','04','عاشورای حسینی'],
                ['05','13','اربعین حسینی'],['05','21','رحلت حضرت رسول اکرم (ص) و شهادت امام حسن مجتبی (ع)'],['05','22','شهادت امام رضا (ع)'],
                ['05','30','شهادت امام حسن عسکری (ع) و آغاز امامت حضرت ولیعصر (عج)'],['06','08','ولادت حضرت رسول اکرم (ص) و ولادت امام جعفر صادق (ع)'],
                ['08','22','شهادت حضرت فاطمه زهرا (س)'],['10','02','ولادت حضرت امام علی (ع) و روز پدر'],['10','16','مبعث حضرت رسول اکرم (ص)'],
                ['11','04','ولادت حضرت قائم (عج)'],['11','22','پیروزی انقلاب اسلامی ایران'],['12','09','شهادت حضرت امام علی (ع)'],['12','19','عید سعید فطر'],
                ['12','20','تعطیل به مناسبت عید سعید فطر'],['12','29','روز ملی شدن صنعت نفت ایران']
            ]
        ];
        $a=$sets[(int)$y]??[];$o=[];foreach($a as $e){$j=sprintf('%04d-%02d-%02d',$y,(int)$e[0],(int)$e[1]);$o[$j.'|'.$e[2]]=['jdate'=>$j,'title'=>$e[2],'is_holiday'=>true,'source'=>'official-reference'];}return$o;
    }

    public static function syncYear($y,$force=false){
        $y=(int)$y;
        /* سال‌های مرجع ۱۴۰۴ و ۱۴۰۵ نباید با داده‌های متناقض سرویس‌های بیرونی جایگزین شوند. */
        $ref=self::fallbackEvents($y);
        if($ref){$c=['year'=>$y,'source'=>'official-reference','synced_at'=>date('c'),'events'=>array_values($ref)];$f=self::file($y);@file_put_contents($f,json_encode($c,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),LOCK_EX);self::$cache[$y]=$c;return true;}
        $f=self::file($y);$c=self::$cache[$y]??(is_file($f)?json_decode((string)@file_get_contents($f),true):null);if(!$force&&is_array($c)&&isset($c['synced_at'])&&time()-strtotime($c['synced_at'])<86400){self::$cache[$y]=$c;return true;}
        $ep=self::env('KHATYAR_CALENDAR_ENDPOINT',self::ENDPOINT);$sep=strpos($ep,'?')===false?'?':'&';$all=[];for($m=0;$m<=12;$m++){ $j=self::json($ep.$sep.'year='.$y.($m?'&month='.$m:''));if($j)self::walk($j,$y,$all);}if(!$all&&!empty($c['events'])){$c['source']=$c['source']??'official-cache';self::$cache[$y]=$c;return true;}if(!$all)return false;$c=['year'=>$y,'source'=>'official-service','synced_at'=>date('c'),'events'=>array_values($all)];@file_put_contents($f,json_encode($c,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),LOCK_EX);self::$cache[$y]=$c;return true;
    }
