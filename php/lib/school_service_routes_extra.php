...
route('POST','/api/school-service/inspections-with-photo',function($p,$b,$u){
 ...
 $gender=in_array($body['school_gender']??'',['دخترانه','پسرانه','دخترانه-پسرانه','نامشخص'],true)?$body['school_gender']:'نامشخص';$dg=in_array($body['driver_gender']??'',['خانم','آقا','نامشخص'],true)?$body['driver_gender']:'نامشخص';$cert=in_array($body['certificate_status']??'',['معتبر','نامعتبر','ارائه نشد'],true)?$body['certificate_status']:'ارائه نشد';
 $front=max(0,(int)($body['passenger_front_count']??0));$rear=max(0,(int)($body['passenger_rear_count']??0));$pass=$front+$rear;$date=_ssv_en($body['violation_date']??'');$time=trim((string)($body['violation_time']??''));if($date!==''&&!preg_match('/^14\d{2}[\/-]\d{1,2}[\/-]\d{1,2}$/',$date))Http::error('تاریخ ثبت تخلف نامعتبر است.',422);if($time!==''&&!preg_match('/^(?:[01]?\d|2[0-3]):[0-5]\d$/',$time))Http::error('ساعت ثبت تخلف نامعتبر است.',422);$locationText=_ssv_norm($body['location_text']??'');if($locationText==='')Http::error('آدرس محل بازرسی الزامی است.',422);[$lat,$lng]=validGeo($body['latitude']??null,$body['longitude']??null);if($lat===null||$lng===null)Http::error('ثبت موقعیت GPS بازرسی الزامی است.',422);
 $f=$_FILES['file']??null;
 ...
 try{
   Db::run("INSERT INTO school_service_inspections(inspector_user_id,client_uuid,educational_district,company_id,school_id,school_gender,plate_three,plate_letter,plate_two,plate_region,iran_code,vehicle_type,vehicle_color,passenger_front_count,passenger_rear_count,passenger_count,driver_gender,certificate_status,violation_date,violation_time,location_text,latitude,longitude,description) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",[(int)$u['id'],$clientUuid?:null,$district,$companyId,$schoolId,$gender,$a,$l,$pc,$pr,'ایران',$vehicleType,$vehicleColor,$front,$rear,$pass,$dg,$cert,$date?:null,$time?:null,$locationText,$lat,$lng,trim((string)($body['description']??''))?:null]);
