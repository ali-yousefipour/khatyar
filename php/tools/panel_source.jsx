...
const db = {
  ...
  login: async (u,p)=>{
    ...
    form.append('device_id',deviceId); form.append('device_type','web');
    ...
    if(!r.ok) throw new Error(d.error||d.message||('ورود ناموفق بود (HTTP '+r.status+')'));
    if(!d.access) throw new Error('پاسخ ورود ناقص است: توکن دریافت نشد.');
...
function RadioSettings(){
  const [data,setData]=useState({channels:[],users:[],roles:[],regions:[],retention_days:1,retention_hours:24}); const [busy,setBusy]=useState(false); const [loadErr,setLoadErr]=useState(''); const [loading,setLoading]=useState(true); const [form,setForm]=useState({id:0,name:'',code:'',description:'',channel_type:'region',match_mode:'OR',max_talk_ms:25000,priority:0,is_active:true,rules:{regions:[],users:[],roles:[]}});
  const api=async(op,opt={})=>{const r=await fetch(`/api/radio-admin-api.php?op=${op}`,{...opt,headers:{...(opt.headers||{}),Authorization:`Bearer ${localStorage.token||''}`,'Content-Type':'application/json'},cache:'no-store'});const d=await r.json().catch(()=>null);if(!r.ok||!d||d.ok===false)throw Error((d&&d.error)||`خطای سرور (کد ${r.status})`);return d;};
  ...
  const load=async()=>{setLoading(true);setLoadErr('');try{setData(await api('bootstrap'));}catch(e){setLoadErr(e.message||'خطای نامشخص در بارگذاری تنظیمات بی‌سیم');}finally{setLoading(false);}}; useEffect(()=>{load()},[]);
  ...
  return(<div style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"var(--paper)"}}>
    <div className="panel" style={{width:380}}>
      ...
      {mode==="login"&&<>
        ...
        <button className="btn p" style={{width:"100%",marginTop:14}} onClick={submit} disabled={busy}>{busy?'در حال ورود…':'ورود'}</button>
