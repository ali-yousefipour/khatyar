...
function fix(){
 document.querySelectorAll('table').forEach(function(t){
  if(!/حضور کل/.test(t.innerText||''))return;
  Array.prototype.forEach.call(t.tBodies||[],function(tb){Array.prototype.forEach.call(tb.rows||[],function(r){
    if(!r.cells.length)return;var key=en(r.cells[0].textContent).replace(/\s/g,'');var title=OFFICIAL[key];if(!title)return;
    var c=r.cells[7];if(c){c.textContent='تعطیل';c.title=title;c.classList.add('kh-official-holiday');}
    r.classList.add('kh-official-holiday-row');
  });});
 });
}
function start(){if(document.getElementById('kh-holiday-render-css'))return;var s=document.createElement('style');s.id='kh-holiday-render-css';s.textContent='.kh-official-holiday,.kh-official-holiday-row .kh-official-holiday{color:#d92d20!important;font-weight:900!important}.kh-official-holiday-row{background:#fff8f7!important}.kh-official-holiday-row:hover{background:#fff1f0!important}';document.head.appendChild(s);fix();new MutationObserver(fix).observe(document.body,{subtree:true,childList:true});}
