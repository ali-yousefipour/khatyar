...
(function () {
  ...
  function addSiteRow(panel, select, key, labelText) {
    ...
    cb.addEventListener('change', function () {
      ...
      savePermission(select.value, cb.checked).catch(function () { cb.checked = !cb.checked; alert('ذخیره دسترسی بی‌سیم ناموفق بود.'); });
    });
    return row;
  }
  function syncSiteAccess() {
    var panel = accessPanel(), select = roleSelect(panel);
    ...
    getSettings().then(function (s) {
      ...
      center.querySelector('input').checked = on; settings.querySelector('input').checked = on;
      center.style.display = 'flex'; settings.style.display = 'flex';
    }).catch(function () { center.style.display = 'flex'; settings.style.display = 'flex'; });
  ...
  function menu(nav, key, label, href) {
    var x = nav.querySelector('[data-khatyar-radio-menu="' + key + '"]');
    if (x) return x;
    x = document.createElement('a');
    x.href = href; x.setAttribute('data-khatyar-radio-menu', key); x.dataset.key = RADIO; x.dataset.view = 'radio'; x.textContent = label;
    x.style.cssText = 'display:block!important;padding:10px 12px;margin:2px 0;border-radius:8px;color:inherit;text-decoration:none;font:inherit;';
    nav.appendChild(x); return x;
  }
  function syncSidebar() {
    var nav = sidebar(); if (!nav) return;
    var center = menu(nav, 'center', '📻 مرکز بی‌سیم', 'radio-admin.html');
    var settings = menu(nav, 'settings', '⚙️ تنظیمات بی‌سیم', 'radio-admin.html#settings');
    /* این‌ها لینک‌های ناوبری هستند؛ مجوز واقعی همچنان توسط API اعمال می‌شود. */
  ...
  var timer = 0;
  new MutationObserver(function () { if (timer) return; timer = setTimeout(function () { timer = 0; run(); }, 180); }).observe(document.documentElement, { childList: true, subtree: true });
})();
