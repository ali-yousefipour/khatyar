...
function fixValue(el){if(!el||!('value' in el))return;if(el.tagName==='TEXTAREA')return;var v=String(el.value||''),x=fixString(v);if(x!==v)el.value=x;}
function scan(root){root=root||document;var w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT),a=[];while(w.nextNode())a.push(w.currentNode);a.forEach(fixText);if(root.querySelectorAll)root.querySelectorAll('input,select').forEach(function(el){if(el.tagName==='SELECT')Array.prototype.forEach.call(el.options||[],function(o){o.textContent=fixString(o.textContent);});else fixValue(el);});}
function loadFixes(){['iran-calendar-ui.js','personnel-selector-fix.js','attendance-holiday-render-fix.js'].forEach(function(name){if(document.querySelector('script[data-kh-global-fix="'+name+'"]'))return;var s=document.createElement('script');s.defer=true;s.src='assets/'+name+'?v=1.3.84';s.setAttribute('data-kh-global-fix',name);document.head.appendChild(s);});}
function start(){scan(document);loadFixes();var timer=0;var ob=new MutationObserver(function(ms){if(timer)clearTimeout(timer);timer=setTimeout(function(){ms.forEach(function(m){if(m.type==='characterData')fixText(m.target);(m.addedNodes||[]).forEach(function(n){if(n.nodeType===3)fixText(n);else if(n.nodeType===1)scan(n);});});},20);});if(document.body)ob.observe(document.body,{subtree:true,childList:true,characterData:true});}
