/* گزارش تردد پرسنل — جستجوی نام/نام خانوادگی با حداقل پایش DOM */
(function(){
'use strict';
...
input.addEventListener('input',filter);input.addEventListener('search',filter);var ob=new MutationObserver(function(){filter();});ob.observe(select,{childList:true});filter();}
function enhance(root){(root||document).querySelectorAll('select').forEach(function(s){if(looks(s))make(s);});}
function start(){enhance(document);var body=document.body;if(!body)return;var stop=false;var ob=new MutationObserver(function(ms){if(stop)return;var found=false;ms.forEach(function(m){Array.prototype.forEach.call(m.addedNodes||[],function(n){if(n.nodeType===1){enhance(n);if(n.querySelector&&n.querySelector('select'))found=true;}});});if(found){stop=true;ob.disconnect();}});ob.observe(body,{childList:true,subtree:true});setTimeout(function(){if(!stop){enhance(document);ob.disconnect();}},2500);}
