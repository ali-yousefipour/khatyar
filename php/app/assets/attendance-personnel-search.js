...
(function(){
  ...
  function personName(u){return String((u.first_name||'')+' '+(u.last_name||'')).replace(/\s+/g,' ').trim();}
  function sortPersian(rows){
    var collator;
    try{collator=new Intl.Collator('fa-IR',{usage:'sort',sensitivity:'base',numeric:true});}
    catch(e){collator=null;}
    return rows.slice().sort(function(a,b){
      var an=personName(a),bn=personName(b);
      if(collator)return collator.compare(an,bn);
      return an.localeCompare(bn,'fa-IR',{sensitivity:'base'});
    });
  }
  async function loadUsers(){if(loading)return;var picker=getPicker();if(!picker)return;var cached=null;try{cached=JSON.parse(localStorage.getItem(USER_CACHE)||'null');}catch(e){}if(cached&&cached.t&&Date.now()-cached.t<USER_TTL&&Array.isArray(cached.rows))applyUsers(picker,sortPersian(cached.rows));loading=true;try{var r=await fetch(USER_URL,{headers:{Authorization:'Bearer '+(localStorage.token||'')},cache:'no-store'});if(!r.ok)throw new Error('users');var rows=normalize(await r.json());if(rows.length){rows=sortPersian(rows);localStorage.setItem(USER_CACHE,JSON.stringify({t:Date.now(),rows:rows}));applyUsers(getPicker()||picker,rows);}}catch(e){}finally{loading=false;}}
  function applyUsers(picker,rows){if(!picker||!picker.select||!rows.length)return;var current=picker.select.value,frag=document.createDocumentFragment(),first=document.createElement('option');first.value='';first.textContent='انتخاب پرسنل…';frag.appendChild(first);rows.forEach(function(u){var o=document.createElement('option');o.value=String(u.id);o.textContent=personName(u);frag.appendChild(o);});picker.select.innerHTML='';picker.select.appendChild(frag);if(current)picker.select.value=current;}
