...
// Default retention is 30 days; override with RADIO_ARCHIVE_RETENTION_DAYS.
ini_set('display_errors','0');
$ROOT=__DIR__.'/..';
require "$ROOT/lib/Db.php";

function radio_cleanup_table($t){
  $r=Db::one("SELECT COUNT(*) c FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=?",[$t]);
  return (int)($r['c']??0)>0;
}
function radio_cleanup_retention(){
  $v=(int)(getenv('RADIO_ARCHIVE_RETENTION_DAYS')?:30);
...
try{
  ...
  $deleted=0;$missing=0;
  foreach($rows as $m){
    $p=__DIR__.'/'.$m['audio_path'];
    if(is_file($p)){if(!@unlink($p))continue;}else{$missing++;}
