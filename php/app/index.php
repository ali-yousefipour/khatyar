...
if (strpos($path, '/api') !== 0) {
  ...
  if ($path === '/app' || $path === '/app.html') {
    ...
    $tag = '<script src="/app-parity.js?v=1" defer></script>';
    if (stripos($html, '</head>') !== false) $html = preg_replace('~</head>~i', $tag . '</head>', $html, 1);
    else $html .= $tag;
...
foreach ($routes as $r) {
  ...
  if (!$r['public']) {
    ...
    if (empty($user['is_admin'])) {
      ...
      if (!empty($mcfg['enabled'])) Http::error((string)($mcfg['message'] ?? '') ?: 'نرم‌افزار و پنل موقتاً برای تعمیرات غیرفعال است. لطفاً بعداً تلاش کنید.', 503);
    }
  ...
  try {
    ...
    if ($debug) Http::json(['error' => 'خطای داخلی سرور', 'detail' => $e->getMessage(), 'line' => $e->getLine(), 'file' => basename($e->getFile())], 500);
    else Http::error('خطای داخلی سرور', 500);
  }
}
