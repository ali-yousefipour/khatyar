...
function pva_ensure_schema(): void {
  ...
  Db::run("CREATE TABLE IF NOT EXISTS personnel_vehicle_assignments (
    user_id BIGINT UNSIGNED NOT NULL,asset_type ENUM('car','motorcycle') NOT NULL,assigned_by BIGINT UNSIGNED NULL,assigned_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(user_id)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
}
...
/* خطیار: منبع اصلی تشخیص «این کاربر خودرو/موتورسیکلت تخصیص‌داده‌شده دارد یا نه» اکنون جدول واقعی تخصیص است
   (که مدیر از پنل وب مشخص می‌کند)، نه حدس‌زدن از روی متن عنوان سمت که با کوچک‌ترین اختلاف نام سمت، اشتباه از آب درمی‌آمد.
   برای سازگاری با نصب‌هایی که هنوز تخصیصی ثبت نکرده‌اند، اگر تخصیصی وجود نداشت، به همان حدسِ قدیمیِ مبتنی بر عنوان سمت برمی‌گردیم. */
function pva_assigned_type(int $user_id): ?string { $r=Db::one('SELECT asset_type FROM personnel_vehicle_assignments WHERE user_id=? LIMIT 1',[$user_id]); return $r?(string)$r['asset_type']:null; }
function pva_effective_type(array $u): ?string { $a=pva_assigned_type((int)$u['id']); if($a!==null)return $a; $k=pva_role_kind((string)($u['role_title']??'')); return $k===false?null:$k; }
...
try {
  ...
  if($op==='assign-list'){
    ...
    $rows=Db::all("SELECT u.id,u.first_name,u.last_name,u.username,r.title role_title,a.asset_type,a.assigned_at
      FROM users u LEFT JOIN roles r ON r.id=u.role_id LEFT JOIN personnel_vehicle_assignments a ON a.user_id=u.id
      WHERE u.is_active=1 ORDER BY (a.asset_type IS NULL),u.first_name,u.last_name,u.id LIMIT 3000");
    pva_json(['ok'=>true,'users'=>$rows]);
  }
  if($op==='assign-set'){
    if(!pva_admin_allowed($u))pva_fail('دسترسی مدیریتی لازم است',403);
    $b=pva_body();$uid=(int)($b['user_id']??0);$type=(string)($b['asset_type']??'');
    if(!$uid)pva_fail('کاربر نامعتبر است');
    if($type===''||$type==='none'){ Db::run('DELETE FROM personnel_vehicle_assignments WHERE user_id=?',[$uid]); pva_json(['ok'=>true,'asset_type'=>null]); }
    if(!in_array($type,['car','motorcycle'],true))pva_fail('نوع وسیله نامعتبر است');
    Db::run('INSERT INTO personnel_vehicle_assignments(user_id,asset_type,assigned_by,assigned_at) VALUES(?,?,?,NOW()) ON DUPLICATE KEY UPDATE asset_type=VALUES(asset_type),assigned_by=VALUES(assigned_by),assigned_at=NOW()',[$uid,$type,$u['id']]);
    pva_json(['ok'=>true,'asset_type'=>$type]);
  }
  ...
  if($op==='save'){
    $type=pva_effective_type($u);$b=pva_body();$reqType=$b['asset_type']??'';
    if($type===null)pva_fail('برای شما خودرو یا موتورسیکلتی تخصیص داده نشده است',403);
    if(!in_array($reqType,['car','motorcycle'],true)||$reqType!==$type)pva_fail('دسترسی ثبت وسیله برای شما مجاز نیست',403);
