...
require dirname(__DIR__, 2).'/lib/Db.php';
require dirname(__DIR__, 2).'/lib/Jwt.php';
...
$h=$_SERVER['HTTP_AUTHORIZATION']??'';if(!preg_match('/Bearer\s+(.+)/i',$h,$m))image_fail(401,'توکن نامعتبر است');
$p=Jwt::verify(trim($m[1]),$CFG['jwt_secret']);if(!$p)image_fail(401,'توکن منقضی یا نامعتبر است');
...
else $row=Db::one("SELECT photo_path,line_id FROM line_station_locations WHERE id=?",[$station]);
...
$base=realpath(dirname(__DIR__).'/uploads/line-locations');$file=realpath(dirname(__DIR__).$rel);
if(!$base||!$file||strpos($file,$base.DIRECTORY_SEPARATOR)!==0||!is_file($file))image_fail(404,'فایل تصویر یافت نشد');
