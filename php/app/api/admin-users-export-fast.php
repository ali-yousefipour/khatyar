<?php
/* خطیار — خروجی Excel کاربران با شماره پرسنلی، سازگار با schemaهای موجود */
...
function uef_auth(){global $CONFIG;$tok=Http::bearer();$p=$tok?Jwt::verify($tok,$CONFIG['jwt_secret']):null;if(!$p||empty($p['sub']))uef_error('توکن منقضی یا نامعتبر است',401);$u=Db::one("SELECT u.id,u.is_active,u.is_admin,r.level,r.title role_title FROM users u JOIN roles r ON r.id=u.role_id WHERE u.id=? LIMIT 1",[$p['sub']]);if(!$u||!(int)$u['is_active'])uef_error('کاربر نامعتبر',401);$dt=$p['dt']??'web';$sess=Db::one("SELECT device_id,revoked_at FROM user_sessions WHERE user_id=? AND device_type=? ORDER BY id DESC LIMIT 1",[$u['id'],$dt]);$unlimited=in_array($u['role_title'],['مدیر کل','رییس اداره بازرسی','نیروی اداری ارشد'],true);if(!$sess||$sess['revoked_at']||(!$unlimited&&$sess['device_id']!==($p['device_id']??'')))uef_error('نشست منقضی یا باطل شده است',401);if(empty($u['is_admin']))uef_error('دسترسی مدیریتی لازم است',403);return $u;}
function uef_e($v){return htmlspecialchars((string)$v,ENT_QUOTES,'UTF-8');}
function uef_has_col($table,$col){return (bool)Db::one("SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=? LIMIT 1",[$table,$col]);}
try {
uef_auth();
$personnelCol=uef_has_col('users','personnel_code')?'u.personnel_code':"''";
$users=Db::all("SELECT u.id,u.username,u.first_name,u.last_name,r.title role_title,u.phone,u.email,u.national_code,$personnelCol personnel_code,u.birth_date,u.marital_status,u.children_count,u.address,u.seniority_start,u.is_active,u.can_send_sms,u.device_model,u.android_version,u.app_version FROM users u LEFT JOIN roles r ON r.id=u.role_id ORDER BY u.id");
...
foreach($users as $r){$pc=trim((string)($r['personnel_code']??''));if($pc===''){foreach($personnelFieldIds as $fid){$v=trim((string)($valMap[$r['id']][$fid]??''));if($v!==''){$pc=$v;break;}}}echo '<tr>';echo '<td>'.uef_e($r['id']).'</td><td>'.uef_e($pc).'</td>';foreach(['username','first_name','last_name','role_title','phone','email','national_code','birth_date','marital_status','children_count','address','seniority_start'] as $k)echo '<td>'.uef_e($r[$k]??'').'</td>';echo '<td>'.((int)$r['is_active']?'بله':'خیر').'</td><td>'.((int)$r['can_send_sms']?'بله':'خیر').'</td>';foreach(['device_model','android_version','app_version'] as $k)echo '<td>'.uef_e($r[$k]??'').'</td>';echo '<td>'.($commitMap[$r['id']]??0).'</td>';foreach($fields as $f)echo '<td>'.uef_e($valMap[$r['id']][$f['id']]??'').'</td>';echo '</tr>';}
echo '</tbody></table></body></html>';exit;
} catch(Throwable $e) {error_log('users-export-fast: '.$e->getMessage().' @ '.$e->getFile().':'.$e->getLine());uef_error('خطای داخلی خروجی کاربران. جزئیات در گزارش خطای سرور ثبت شد.',500);}
