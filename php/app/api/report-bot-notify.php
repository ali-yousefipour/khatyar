...
function rb_report_context($reportId) {
  ...
  try {
    $r=Db::one("SELECT r.*, u.first_name sender_first_name, u.last_name sender_last_name, u.phone sender_phone FROM reports r LEFT JOIN users u ON u.id=r.sender_id WHERE r.id=? LIMIT 1",[$reportId]);
    if(!$r) return null;
    $routes=[];
    try {
      $rows=Db::all("SELECT rr.id,rr.to_user_id,rr.action,rr.note,rr.actor_id,rr.created_at,a.first_name actor_first_name,a.last_name actor_last_name,t.first_name target_first_name,t.last_name target_last_name FROM report_routes rr LEFT JOIN users a ON a.id=rr.actor_id LEFT JOIN users t ON t.id=rr.to_user_id WHERE rr.report_id=? ORDER BY rr.created_at ASC, rr.id ASC",[$reportId]);
...
function rb_report_text($fallbackUser,$subject,$body,$reportId) {
  ...
  $parts=['📩 گزارش جدید برای بررسی','','📌 موضوع: '.($title!==''?$title:'---'),'👤 گزارش‌دهنده: '.$sender,'🕐 زمان ارسال: '.rb_format_datetime($created),'','📝 متن گزارش:',$content!==''?$content:'---'];
  $items=[];
  foreach($routes as $row){
    ...
    if($label==='اقدام' && $target==='' && $note==='') continue;
    $line='👤 '.$actor.' | '.$label.' | 🕐 '.rb_format_datetime($row['created_at']??'');
    if($target!=='') $line.='\n➡️ ارجاع به: '.$target;
    if($note!=='') $line.='\n📝 یادداشت: '.$note;
    $items[]=$line;
  }
  if($items){ $parts[]=''; $parts[]='📌 ارجاعات و یادداشت‌ها:'; foreach($items as $i=>$item){ if($i>0) $parts[]='────────────────'; $parts[]=$item; } }
  return implode("\n",$parts);
}
$u=rb_user();
...
$attachments=[];
foreach(array_slice((array)($b['attachments']??[]),0,5) as $i=>$a){ $f=rb_file($a['data']??null,$a['name']??('attachment-'.$i),$i); if($f)$attachments[]=$f; }
$out=[];
try{
  ...
  if($chat){
    ...
    foreach($attachments as $f){ $isImage=(strpos($f['mime'],'image/')===0); $method=$isImage?'sendPhoto':'sendDocument'; $payload=['chat_id'=>$chat,$isImage?'photo':'document'=>$f['url'],'caption'=>($f['name']??'پیوست گزارش')]; $out['bale']['attachments'][]=BaleBot::request($method,$payload); }
...
foreach(['telegram','eitaa'] as $platform){
  try{
    ...
    foreach($attachments as $f){ $isImage=(strpos($f['mime'],'image/')===0); $method=$isImage?'sendPhoto':'sendDocument'; $payload=['chat_id'=>$sub['chat_id'],$isImage?'photo':'document'=>$f['url'],'caption'=>($f['name']??'پیوست گزارش')]; $out[$platform]['attachments'][]=MessengerBot::request($platform,$method,$payload); }
