...
-- این Migration در صورت نبود Migration قبلی نیز مستقل قابل اجرا است.
...
SET @sql=(SELECT IF(COUNT(*)=0,'ALTER TABLE radio_user_settings ADD COLUMN listen_all TINYINT(1) NOT NULL DEFAULT 0','SELECT 1') FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=@db AND TABLE_NAME='radio_user_settings' AND COLUMN_NAME='listen_all'); PREPARE s0 FROM @sql; EXECUTE s0; DEALLOCATE PREPARE s0;
