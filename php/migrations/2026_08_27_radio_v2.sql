-- KhatYar Radio v2: server-enforced channel rules, presence and audit log.
-- MySQL/MariaDB compatible and safe to run repeatedly.

ALTER TABLE radio_channels
  ADD COLUMN channel_type VARCHAR(20) NOT NULL DEFAULT 'custom',
  ADD COLUMN match_mode VARCHAR(3) NOT NULL DEFAULT 'OR',
  ADD COLUMN max_talk_ms INT UNSIGNED NOT NULL DEFAULT 25000,
...
CREATE TABLE IF NOT EXISTS radio_channel_regions (
  ...
  KEY idx_radio_cr_region (region_id),
...
CREATE TABLE IF NOT EXISTS radio_channel_roles (
  ...
  KEY idx_radio_crole_role (role_id),
...
CREATE TABLE IF NOT EXISTS radio_presence (
  ...
  PRIMARY KEY (channel_id, user_id),
  KEY idx_radio_presence_seen (channel_id, last_seen_at),
...
CREATE TABLE IF NOT EXISTS radio_logs (
  ...
  PRIMARY KEY (id),
  KEY idx_radio_logs_channel (channel_id, id),
  KEY idx_radio_logs_user (user_id, id),
  CONSTRAINT fk_radio_logs_channel FOREIGN KEY (channel_id) REFERENCES radio_channels(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Convert the four original channels to the new schema without changing their meaning.
