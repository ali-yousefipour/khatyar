...
function Invoke-GradleRelease {
    ...
    $psi.RedirectStandardOutput = $false
    $psi.RedirectStandardError = $false
    # Repository/plugin configuration is generated by Expo and the withMyketMirror
    # config plugin. Do not pass myket.init.gradle here because Gradle applies an
    # init script to every included build, including React Native's gradle-plugin.
...
try {
    ...
    Write-Stage 72 ('Building release ' + $ArtifactType + ' with Expo-generated Myket repository configuration')
