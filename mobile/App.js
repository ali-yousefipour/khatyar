  ...
  React.useEffect(() => {
    ...
    const schedule = (baseDelay) => {
      ...
      [0, 500, 1000, 1500].forEach((extra, index) => {
        const cancel = afterUiReady(() => {
          if (active) setStage((current) => Math.max(current, index + 1));
        }, baseDelay + extra);
        cancels.push(cancel);
      });
    };
...
function Routes() {
  ...
  if (loading || (user && subscriptionLoading)) {
    return <ActivityIndicator fullScreen size={120} message="در حال دریافت اطلاعات کاربر…" />;
  }
  if (user && (user.must_change_pw || user.must_setup)) {
    return <FirstSetupScreen onDone={refreshUser} />;
  }
  if (user && user.must_renew) {
    return <PeriodicRenewScreen onDone={refreshUser} />;
  }
  if (user && subscription?.enabled && !subscription?.active) {
    return <SubscriptionScreen onActivated={setSubscription} />;
  }
...
export default function App() {
  ...
  const runVersionCheck = React.useCallback(async () => {
    try { setUpdateInfo(await checkVersion()); }
    catch (e) { setUpdateInfo(null); }
  }, []);
