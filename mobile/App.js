...
function Routes(){
  ...
  React.useEffect(()=>{
    ...
    request('/subscription/status',{noStore:true}).then(data=>{if(alive)setSubscription(data?.subscription??null)}).catch(()=>{if(alive)setSubscription(null)}).finally(()=>{if(alive)setSubscriptionLoading(false)});
    return()=>{alive=false};
  },[user?.id]);
