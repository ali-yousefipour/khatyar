...
const stackOptions = {
  ...
  headerLeft: ({ navigation }) => (
    <TouchableOpacity onPress={() => openDrawer(navigation)} style={{ paddingHorizontal: 10 }} hitSlop={8}>
...
function DeferredRuntimeServices() {
  ...
  React.useEffect(() => {
    ...
    const schedule = (baseDelay) => {
      cancels.forEach((cancel) => { try { cancel?.(); } catch (_) {} });
      cancels = [];
      [0,500,1000,1500].forEach((extra,index) => {
        const cancel = afterUiReady(() => { if (active) setStage(current => Math.max(current, index + 1)); }, baseDelay + extra);
        cancels.push(cancel);
      });
    };
    schedule(1800);
...
function MainStack() {
  ...
  React.useEffect(() => {
    let alive = true;
    if (!user) { setSubscription(null); setSubscriptionLoading(false); return undefined; }
    setSubscriptionLoading(true);
    request('/subscription/status', { noStore: true })
      .then(data => { if (alive) setSubscription(data?.subscription ?? null); })
      .catch(() => { if (alive) setSubscription(null); })
      .finally(() => { if (alive) setSubscriptionLoading(false); });
    return () => { alive = false; };
  }, [user?.id]);
  ...
  return (
    <Stack.Navigator screenOptions={stackOptions}>
      {!user ? <Stack.Screen name="Login" component={LoginScreen} options={{ headerShown: false }} /> : <>
        ...
        <Stack.Screen name="TeamReport" component={TeamReportScreen} options={{ title: 'زیرمجموعه من' }} />
  ...
  return (
    <Drawer.Navigator
      screenOptions={{
        ...
        drawerType: 'front',
        drawerStyle: { width: '84%', backgroundColor: C.paper },
        swipeEdgeWidth: 40,
      }}
...
export default function App() {
  ...
  const runVersionCheck = React.useCallback(async () => { try { setUpdateInfo(await checkVersion()); } catch (e) { setUpdateInfo(null); } }, []);

  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try { await Font.loadAsync({ [FONT.regular]: require('./assets/fonts/Vazirmatn-Regular.ttf'), [FONT.bold]: require('./assets/fonts/Vazirmatn-Bold.ttf') }); } catch (e) {}
      try { await loadApiBase(); } catch (e) {}
      if (mounted) setReady(true);
      SplashScreen.hideAsync().catch(() => {});
      runVersionCheck().catch(() => {});
      flushCrashReports().catch(() => {});
      try { require('./src/updater').checkForUpdate(false); } catch (e) {}
    })();
  ...
  if (updateInfo?.required) return <ErrorBoundary><StatusBar style="light" /><UpdateScreen info={updateInfo} onRecheck={runVersionCheck} /></ErrorBoundary>;
