...
import { Alert, Image, Modal, ScrollView, StyleSheet, Text, TouchableOpacity, View, useWindowDimensions } from 'react-native';
...
export default function DashboardScreen({ navigation }) {
  ...
  const canShow = (m) => { if (m.r === 'Help') return false; if (m.needs && !access[m.needs]) return false; if (allowed === null) return true; if (m.r === 'StationCapture' || m.r === 'MyStations') return ['StationCapture', 'MyStations', 'LineLocation'].some((k) => allowed.includes(k)); return allowed.includes(m.r); };
...
const s = StyleSheet.create({
  ...
  grid: { flexDirection: 'row-reverse', paddingHorizontal: 12, gap: 10 },
  card: { width: 84, backgroundColor: '#fff', borderColor: C.line, borderWidth: 1, borderRadius: 18, paddingVertical: 12, paddingHorizontal: 6, alignItems: 'center', justifyContent: 'flex-start' },
