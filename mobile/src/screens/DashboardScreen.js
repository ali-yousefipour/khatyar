  ...
  return (
    <ScrollView style={{ backgroundColor: C.paper }} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      {lead && lead.groups && lead.groups.some(g => (g.top || []).length > 0) ? (
        <View>
          <TouchableOpacity onPress={() => setLeadOpen((v) => !v)} style={s.leadHeader} activeOpacity={0.7}>
            <Text style={[s.sectionTitle, { marginHorizontal: 0, marginTop: 0, marginBottom: 0 }]}>پرکارترین و کم‌کارترین نیروها{lead.zone_scope === 'own' ? ' (منطقهٔ شما)' : ''}</Text>
            <Text style={s.leadChevron}>{leadOpen ? '▲' : '▼'}</Text>
          </TouchableOpacity>
          {leadOpen && (
...
const s = StyleSheet.create({
  ...
  leadHeader: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', marginHorizontal: 16, marginTop: 18, marginBottom: 8 },
  leadChevron: { color: C.muted, fontSize: 13 },
