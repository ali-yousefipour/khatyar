...
export default function DashboardScreen({ navigation }) {
  ...
  useEffect(() => {
    if (!user) return;
    request('/school-service/access', { noStore: true }).then((r) => setSchoolServiceAllowed(r?.allowed === true)).catch(() => setSchoolServiceAllowed(false));
  }, [user?.id]);
  ...
  const canShow = (m) => { if (m.r === 'Help') return false; if (m.needs === 'schoolservice' && !schoolServiceAllowed) return false; if (m.needs && m.needs !== 'schoolservice' && !access[m.needs]) return false; if (allowed === null) return true; if (m.r === 'StationCapture' || m.r === 'MyStations') return ['StationCapture', 'MyStations', 'LineLocation'].some((k) => allowed.includes(k)); return allowed.includes(m.r); };
  const visibleMenu = MENU.filter((m) => (m.r !== 'Sms' && m.r !== 'MySms' && m.r !== 'BotMessages') || !!user?.can_send_sms || m.r === 'BotMessages').filter(canShow).filter((m) => menuTab === 'all' || categoryOf(m.r) === menuTab).filter((m) => { const q = String(menuSearch || '').trim().toLocaleLowerCase(); return !q || String(m.t || '').toLocaleLowerCase().includes(q); });
...
const s = StyleSheet.create({
  ...
  menuSearch: { marginHorizontal: 14, marginBottom: 10, minHeight: 46, borderWidth: 1, borderColor: C.line, borderRadius: 12, backgroundColor: '#fff', paddingHorizontal: 12, fontFamily: FONT.regular, fontSize: 13, color: C.ink, textAlign: 'right', writingDirection: 'rtl' },
