...
function TimePicker({ visible, hour, minute, onClose, onSelect }) {
  ...
  return <Modal visible={!!visible} transparent animationType="slide" onRequestClose={onClose}><View style={st.timeBg}><View style={st.timeCard}>
    ...
    <View style={st.timeCols}><ScrollView persistentScrollbar={true} style={st.timeCol}>{hours.map(x=><TouchableOpacity key={x} style={[st.timeItem,x===hour&&st.timeOn]} onPress={()=>onSelect(x,minute)}><Text style={[st.timeTxt,x===hour&&st.timeTxtOn]}>{fa(x)}</Text></TouchableOpacity>)}</ScrollView>
    <Text style={st.timeColon}>:</Text><ScrollView persistentScrollbar={true} style={st.timeCol}>{mins.map(x=><TouchableOpacity key={x} style={[st.timeItem,x===minute&&st.timeOn]} onPress={()=>onSelect(hour,x)}><Text style={[st.timeTxt,x===minute&&st.timeTxtOn]}>{fa(x)}</Text></TouchableOpacity>)}</ScrollView></View>
