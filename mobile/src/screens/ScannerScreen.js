...
import { View, Text, TouchableOpacity, StyleSheet, Modal, TextInput, Alert, Image, ScrollView, useWindowDimensions } from 'react-native';
...
export default function ScannerScreen({ navigation, route }) {
  ...
  const scanFrame = mode === 'plate'
    ? { width: Math.min(screenW * 0.92, 430), height: Math.min(screenW * 0.92 / 4.6, 110) }
    : { width: Math.min(screenW * 0.9, 420), height: Math.min(screenW * 0.9 / 1.58, screenH * 0.38) }; // 'plate' | 'national_id' | 'auto'
    ...
    <View style={s.screen}>
      <View style={s.cameraStage}>
        <CameraView
          ...
          onMountError={(e) => setHint(e?.message || 'راه‌اندازی دوربین ناموفق بود')}
        />
        {!cameraReady ? <View style={s.cameraLoading} pointerEvents="none"><ActivityIndicator color="#fff"/><Text style={s.cameraLoadingText}>در حال فعال‌سازی دوربین…</Text></View> : null}
        <View style={s.overlay} pointerEvents="none"><View style={[s.frame, scanFrame, mode === 'plate' ? s.plateFrame : s.cardFrame]}><View style={s.cornerTL}/><View style={s.cornerTR}/><View style={s.cornerBL}/><View style={s.cornerBR}/></View></View>
      </View>
      <View style={s.bottom}>
...
const s = StyleSheet.create({
  ...
  cameraStage: { flex: 1, position: 'relative', overflow: 'hidden', backgroundColor: '#000' },
  cameraPreview: { ...StyleSheet.absoluteFillObject, width: '100%', height: '100%' },
  cameraLoading: { ...StyleSheet.absoluteFillObject, alignItems: 'center', justifyContent: 'center', backgroundColor: '#000' },
  cameraLoadingText: { color:'#fff', fontFamily:FONT.regular, marginTop:10 },
  ...
  frame: { borderWidth: 2, borderColor: C.taxi, borderRadius: 16, backgroundColor: 'transparent' },
  plateFrame: { borderColor: '#f5c518', borderRadius: 10 },
  cardFrame: { borderColor: '#f5c518', borderRadius: 18 },
  cornerTL: { position:'absolute', top:-3, left:-3, width:28, height:28, borderTopWidth:5, borderLeftWidth:5, borderColor:'#fff', borderTopLeftRadius:12 },
  cornerTR: { position:'absolute', top:-3, right:-3, width:28, height:28, borderTopWidth:5, borderRightWidth:5, borderColor:'#fff', borderTopRightRadius:12 },
  cornerBL: { position:'absolute', bottom:-3, left:-3, width:28, height:28, borderBottomWidth:5, borderLeftWidth:5, borderColor:'#fff', borderBottomLeftRadius:12 },
  cornerBR: { position:'absolute', bottom:-3, right:-3, width:28, height:28, borderBottomWidth:5, borderRightWidth:5, borderColor:'#fff', borderBottomRightRadius:12 },
  bottom: { position: 'relative', padding: 18, alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.55)' },
