...
export async function getAccuratePosition({samples=2,timeoutMs=7000,desiredAccuracy=20,maxAccuracy=80}={}){let best=null;for(let i=0;i<samples;i++){try{const p=await Promise.race([Location.getCurrentPositionAsync({accuracy:Location.Accuracy.High,mayShowUserSettingsDialog:true}),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),timeoutMs))]);if(p?.coords){const a=p.coords.accuracy??9999;if(!best||a<(best.coords.accuracy??9999))best=p;if(a<=desiredAccuracy)break;}}catch{}}const a=best?.coords?.accuracy??9999;return best&&a<=maxAccuracy?best:null;}
export async function getFastPosition({maxAgeMs=30000,timeoutMs=5000,maxAccuracy=100}={}){try{const last=await Location.getLastKnownPositionAsync({maxAge:maxAgeMs,requiredAccuracy:maxAccuracy});if(last?.coords){Location.getCurrentPositionAsync({accuracy:Location.Accuracy.High,mayShowUserSettingsDialog:true}).catch(()=>{});return last;}}catch{}try{const p=await Promise.race([Location.getCurrentPositionAsync({accuracy:Location.Accuracy.Balanced,mayShowUserSettingsDialog:true}),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),timeoutMs))]);const a=p?.coords?.accuracy??9999;if(p?.coords&&a<=maxAccuracy)return p;}catch{}return null;}
export async function getGsmPosition({timeoutMs=5000,maxAccuracy=100}={}){try{const p=await Promise.race([Location.getCurrentPositionAsync({accuracy:Location.Accuracy.Lowest}),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),timeoutMs))]);const a=p?.coords?.accuracy??9999;if(p?.coords&&a<=maxAccuracy)return{coords:p.coords,timestamp:p.timestamp,viaGsm:true};}catch{}return null;}
