...
export default function PermissionGuard({children}){
  ...
  return <View style={{flex:1}}>{children}
    {cameraExplanation&&<View style={s.overlay}><ScrollView persistentScrollbar={true} contentContainerStyle={s.box}><Text style={s.icon}>📷</Text><Text style={s.title}>مجوز استفاده از دوربین</Text><Text style={s.sub}>دوربین فقط برای صحت‌سنجی حضور و فقط پس از شروع فرایند صحت‌سنجی استفاده می‌شود. خارج از آن فرایند، برنامه از دوربین استفاده نمی‌کند.</Text><TouchableOpacity style={s.btn} onPress={handleCameraExplanation}><Text style={s.btnTxt}>تأیید و درخواست دسترسی</Text></TouchableOpacity></ScrollView></View>}
    {micExplanation&&<View style={s.overlay}><ScrollView persistentScrollbar={true} contentContainerStyle={s.box}><Text style={s.icon}>🎙️</Text><Text style={s.title}>مجوز استفاده از میکروفون</Text><Text style={s.sub}>میکروفون فقط برای صحبت در بی‌سیم و فقط هنگام فشردن دکمهٔ صحبت استفاده می‌شود. خارج از آن، برنامه از میکروفون استفاده نمی‌کند.</Text><TouchableOpacity style={s.btn} onPress={handleMicExplanation}><Text style={s.btnTxt}>تأیید و درخواست دسترسی</Text></TouchableOpacity></ScrollView></View>}
    {blocking&&<View style={s.overlay}><ScrollView persistentScrollbar={true} contentContainerStyle={s.box}><Text style={s.icon}>{blocking.icon}</Text><Text style={s.title}>{blocking.title}</Text><Text style={s.sub}>{blocking.text}</Text><TouchableOpacity style={s.btn} disabled={checking} onPress={async()=>{if(runtimeIssue.type==='notifications'){const r=await callModuleAsync(Notifications,'requestPermissionsAsync',null);if(r?.status==='granted'){check();return}}else if(runtimeIssue.type==='vpn'){await openVpnSettings()}else if(runtimeIssue.type==='gps'){await openLocationSettings()}else{try{await Linking.openSettings()}catch{}}check()}}><Text style={s.btnTxt}>{blocking.button}</Text></TouchableOpacity></ScrollView></View>}
  </View>;
}
