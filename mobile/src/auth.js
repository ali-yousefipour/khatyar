...
function roleStars(user={}){const role=String(user.role??user.role_name??user.position??user.job_title??user.post??'').trim();if(!role)return 0;if(role.includes('سربازرس'))return 4;if(role.includes('بازرس'))return 2;if(role.includes('سرپرست خط')||role.includes('ناظر خط')||role.includes('اپراتور')||role.includes('مسئول خط'))return 1;return 0;}
function normalizeUser(user){if(!user||typeof user!=='object')return user;const explicit=[user.rank_stars,user.role_stars,user.degree_stars,user.stars].find(v=>v!==null&&v!==undefined&&v!=='');const explicitStars=Number(explicit);const stars=Number.isFinite(explicitStars)&&explicitStars>0?Math.max(0,Math.min(5,Math.trunc(explicitStars))):roleStars(user);return{...user,rank_stars:stars};}
...
export function AuthProvider({children}){
 ...
 useEffect(()=>{(async()=>{try{const token=await loadTokens();if(token){try{const me=await request('/auth/me');setUser(normalizeUser(me.user));beginActivity()}catch{await clearTokens()}}}catch{}finally{setLoading(false)}})()},[]);
 ...
 async function login(username,password,remember){const device_id=await getDeviceId();const sig=await securitySignals();const d=await loginRequest({username,password,device_id,device_type:'android',device_model:getDeviceModel(),...sig});await setTokens(d.access,d.refresh);await SecureStore.setItemAsync('remember','1');setUser(normalizeUser(d.user));beginActivity();return d.user;}
 async function loginOtpRequest(mobile){await requestLoginOtp(mobile)}
 async function loginOtpVerify(mobile,code){const device_id=await getDeviceId();const d=await verifyLoginOtp({mobile,code,device_id,device_type:'android',device_model:getDeviceModel()});await setTokens(d.access,d.refresh);await SecureStore.setItemAsync('remember','1');setUser(normalizeUser(d.user));beginActivity();return d.user;}
 async function logout(){try{await request('/auth/logout',{method:'POST'})}catch(e){throw new Error((e&&e.message)||'خروج ممکن نیست')}try{sendTelemetry('session_end');stopTelemetry();stopVpnMonitor();stopHealthMonitor();stopNotifyPolling()}catch{}try{await stopTracking()}catch{}await clearTokens();await SecureStore.setItemAsync('remember','0');setUser(null);}
 async function refreshUser(){try{const me=await request('/auth/me');const normalized=normalizeUser(me.user);setUser(normalized);return normalized}catch{return null}}
