...
async function refreshAccess() {
  ...
  let r;
  try {
    const form = new URLSearchParams(); form.append('refresh', refresh);
    r = await fetchTimeout(`${apiBase()}/session/renew`, { method:'POST', headers:{'content-type':'application/x-www-form-urlencoded; charset=UTF-8','accept':'application/json'}, body:form.toString() });
...
async function _sessionFormPost(path, body, { timeoutMs = 25000 } = {}) {
  ...
  try{data=text?JSON.parse(text):{}}catch(_){throw new Error(`پاسخ سرور JSON نیست (HTTP ${res.status}).`)}
...
// Multipart uploads need the original file bytes and therefore remain explicit operations.
...
function isQueueableWrite(path,method){
  if(!['POST','PUT','PATCH','DELETE'].includes(String(method).toUpperCase()))return false;
  const p=String(path||'');
  // عملیات احراز هویت و بی‌سیم زنده نباید به صف عمومی تبدیل شوند.
  if(/^\/session\//.test(p)||/^\/auth\//.test(p)||p.includes('radio-api-v2.php'))return false;
  return true;
}
...
export async function request(path,{method='GET',body,auth=true,retry=true,noStore=false,timeoutMs,queueIfOffline=true}={}){
  ...
  const queueable=queueIfOffline&&isQueueableWrite(path,upper); const sendBody=queueable?eventBody(body):body;
  try{
    ...
    const res=await fetchTimeout(`${apiBase()}${path}`,{method:upper,headers,body:sendBody!==undefined?JSON.stringify(sendBody):undefined},effectiveTimeout);
    ...
    if(queueable&&queueIfOffline){try{return await queueWrite(path,sendBody,upper)}catch(_){}
    }
...
export async function postOrQueue(path,body,type=null,opts={}){
  ...
  catch(e){if(String(e?.message||'').includes('اتصال')||String(e?.message||'سرور')||String(e?.message||'زمان انتظار')){await enqueue({path,body:sendBody,type:type||path,client_uuid:sendBody?.client_uuid,client_time:eventMs});return{queued:true,client_time:eventMs,client_uuid:sendBody?.client_uuid}}throw e}
}
