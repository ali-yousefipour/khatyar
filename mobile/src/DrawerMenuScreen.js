    ...
    <View style={s.page}>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        {ITEMS.map(([route, title], i) => (
          <TouchableOpacity key={route} style={[s.item, i === ITEMS.length - 1 && s.helpItem]} onPress={() => go(route)} activeOpacity={0.82}>
...
const s = StyleSheet.create({
  ...
  iconText: { color: C.brand, fontSize: 20, fontFamily: FONT.bold },
  itemText: { flex: 1, color: C.ink, fontFamily: FONT.bold, fontSize: 13, textAlign: 'right' },
  helpItem: { marginTop: 8, borderColor: C.brand },
});
