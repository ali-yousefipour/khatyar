...
function Ensure-Node {
  ...
  Install-WingetPackage 'OpenJS.NodeJS.LTS' 'Node.js 22 LTS'
  Refresh-ProcessPath
  $fallback = 'C:\Program Files\nodejs'
  if (Test-Path "$fallback\node.exe") { $env:Path = "$fallback;$env:Path" }
  $raw2 = (& node.exe --version 2>$null).Trim()
  if ($raw2 -notmatch '^v22\.') { throw "Node.js 22.x installation could not be verified. Detected: $raw2" }
  Write-Ok "Node.js $raw2 ready."
}
...
function Ensure-Jdk {
  ...
  if ($java) {
    ...
    if ($ok) { Write-Ok 'JDK 17 detected.' }
    else { Write-Warn 'Installed Java is not JDK 17.' }
  ...
  if (-not $home -or -not (Test-Path (Join-Path $home 'bin\java.exe'))) {
    throw 'JDK 17 was installed but JAVA_HOME could not be determined.'
  }
  $env:JAVA_HOME = $home
...
function Bootstrap-AndroidCommandLineTools([string]$SdkRoot) {
  ...
  $zip = Join-Path $env:TEMP 'khatyar-android-cmdline-tools.zip'
  ...
  try {
    ...
    if (-not (Test-Path $zip) -or (Get-Item $zip).Length -lt 10MB) { throw 'Downloaded Android command-line tools archive is invalid.' }
    ...
    throw "Android SDK command-line tools download/install failed: $($_.Exception.Message)"
  }
}
...
function Ensure-AndroidSdk {
  $sdk = Find-AndroidSdk
  if (-not $sdk) {
    Write-Warn 'Android SDK not detected; attempting automatic Android Studio installation.'
    if (Test-Winget) {
...
Write-Host '============================================================' -ForegroundColor Cyan
Write-Stage 'Checking Windows package manager'
if (-not (Test-Winget)) { Write-Warn 'winget is not available. Existing installations will be used; missing tools cannot be installed automatically.' }
