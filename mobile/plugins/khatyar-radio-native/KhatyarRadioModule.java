...
public final class KhatyarRadioModule extends ReactContextBaseJavaModule {
  ...
  private final Runnable foregroundMonitor=new Runnable(){@Override public void run(){if(!foregroundRequested){releaseForegroundEnhancer();return;}try{if(Build.VERSION.SDK_INT>=26&&hasActiveAppPlayback())ensureForegroundEnhancer();else releaseForegroundEnhancer();}catch(Throwable ignored){}handler.postDelayed(this,180L);}};
  ...
  private boolean hasActiveAppPlayback(){if(Build.VERSION.SDK_INT<26)return false;try{AudioManager am=(AudioManager)context.getSystemService(Context.AUDIO_SERVICE);if(am==null)return false;int uid=android.os.Process.myUid();List<AudioPlaybackConfiguration> active=am.getActivePlaybackConfigurations();for(AudioPlaybackConfiguration cfg:active){if(cfg!=null&&cfg.getClientUid()==uid)return true;}}catch(Throwable ignored){}return false;}
  private synchronized void ensureForegroundEnhancer(){int gain=KhatyarRadioService.clampGainMb(context.getSharedPreferences(KhatyarRadioService.PREFS,Context.MODE_PRIVATE).getInt("amplificationGainMb",600));if(gain<=0){releaseForegroundEnhancer();return;}try{if(foregroundEnhancer==null)foregroundEnhancer=new LoudnessEnhancer(0);foregroundEnhancer.setTargetGain(gain);foregroundEnhancer.setEnabled(true);}catch(Throwable ignored){releaseForegroundEnhancer();}}
  ...
  private synchronized void releaseForegroundEnhancer(){if(foregroundEnhancer!=null){try{foregroundEnhancer.setEnabled(false);}catch(Throwable ignored){}try{foregroundEnhancer.release();}catch(Throwable ignored){}foregroundEnhancer=null;}}
