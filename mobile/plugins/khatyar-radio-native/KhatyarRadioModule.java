...
public final class KhatyarRadioModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod public void configure(String token,String baseUrl,double userId,double channelId,boolean enabled,double lastMessageId,Promise promise){try{Context app=context.getApplicationContext();android.content.SharedPreferences p=app.getSharedPreferences(KhatyarRadioService.PREFS,Context.MODE_PRIVATE);long oldChannel=p.getLong("channelId",0L);boolean channelChanged=oldChannel!=(long)channelId;p.edit().putString("token",token==null?"":token).putString("baseUrl",baseUrl==null?"":baseUrl).putLong("userId",(long)userId).putLong("channelId",(long)channelId).putBoolean("enabled",enabled).apply();if(channelChanged){
    /* خطیار: قبلاً این‌جا lastId همیشه صفر می‌شد (یعنی «همه‌چیز از اول کانال جدید تازه است»). اگر درست همان لحظهٔ تعویض کانال
       اپ در پس‌زمینه بود (حتی برای یک لحظهٔ کوتاه، مثلاً درست وسط همین تعویض)، سرویس نیتیو با poll بعدی‌اش تمام پیام‌های
       تاریخی آن کانال را «پیام تازه» تشخیص می‌داد و همه را پشت سر هم پخش می‌کرد — همان پخش مجدد پیام‌های قدیمی کانال.
       حالا لحظهٔ تعویض، آخرین شناسهٔ پیامِ واقعیِ همان کانال (که جاوااسکریپت از قبل می‌داند) به نیتیو هم داده می‌شود. */
    p.edit().putLong("lastId",Math.max(0L,(long)lastMessageId)).putBoolean("initialized",false).apply();app.stopService(new Intent(app,KhatyarRadioService.class));}if(enabled&&channelId>0&&token!=null&&!token.isEmpty()){Intent in=new Intent(app,KhatyarRadioService.class);if(Build.VERSION.SDK_INT>=26)app.startForegroundService(in);else app.startService(in);}else app.stopService(new Intent(app,KhatyarRadioService.class));promise.resolve(true);}catch(Throwable e){promise.reject("RADIO_NATIVE",e);}}
