...
public final class KhatyarRadioModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod public void setListenAll(boolean listenAll,Promise promise){try{Context app=context.getApplicationContext();android.content.SharedPreferences p=app.getSharedPreferences(KhatyarRadioService.PREFS,Context.MODE_PRIVATE);boolean old=p.getBoolean("listenAll",false);if(old!=listenAll){p.edit().putBoolean("listenAll",listenAll).putBoolean("allInitialized",false).putLong("listenAllSince",System.currentTimeMillis()).apply();}else{p.edit().putBoolean("listenAll",listenAll).apply();}promise.resolve(true);}catch(Throwable e){promise.reject("RADIO_NATIVE",e);}}
  ...
  @ReactMethod public void configure(String token,String baseUrl,double userId,double channelId,boolean enabled,double lastMessageId,Promise promise){try{Context app=context.getApplicationContext();android.content.SharedPreferences p=app.getSharedPreferences(KhatyarRadioService.PREFS,Context.MODE_PRIVATE);long oldChannel=p.getLong("channelId",0L);boolean channelChanged=oldChannel!=(long)channelId;p.edit().putString("token",token==null?"":token).putString("baseUrl",baseUrl==null?"":baseUrl).putLong("userId",(long)userId).putLong("channelId",(long)channelId).putBoolean("enabled",enabled).apply();if(channelChanged){p.edit().putLong("lastId",Math.max(0L,(long)lastMessageId)).putBoolean("initialized",false).putBoolean("allInitialized",false).putLong("listenAllSince",System.currentTimeMillis()).apply();app.stopService(new Intent(app,KhatyarRadioService.class));}if(enabled&&channelId>0&&token!=null&&!token.isEmpty()){Intent in=new Intent(app,KhatyarRadioService.class);if(Build.VERSION.SDK_INT>=26)app.startForegroundService(in);else app.startService(in);}else app.stopService(new Intent(app,KhatyarRadioService.class));promise.resolve(true);}catch(Throwable e){promise.reject("RADIO_NATIVE",e);}}
