...
function withRadioPtt(config) {
  ...
  config = withDangerousMod(config, ['android', async c => {
    ...
    for(const file of found){let s=fs.readFileSync(file,'utf8');if(!s.includes('KhatyarRadioPttBridge.emit')){const importLine=`import ${radioPkg}.KhatyarRadioPttBridge\n`;if(!s.includes(importLine.trim())){const packageMatch=s.match(/^package [^\n]+\n/);if(packageMatch)s=s.slice(0,packageMatch[0].length)+importLine+s.slice(packageMatch[0].length);else s=importLine+s}if(file.endsWith('.kt')){const methods=`\n\n  private var khatyarVolumePttActive = false\n\n  override fun dispatchKeyEvent(event: android.view.KeyEvent): Boolean {\n    if (event.keyCode == android.view.KeyEvent.KEYCODE_VOLUME_UP) {\n      if (event.action == android.view.KeyEvent.ACTION_DOWN) {\n        if (event.repeatCount == 0 && !khatyarVolumePttActive && KhatyarRadioPttBridge.volumePttAllowed(this)) {\n          khatyarVolumePttActive = true\n          KhatyarRadioPttBridge.emit(this, \"down\", \"volume_up\")\n          return true\n        }\n        if (khatyarVolumePttActive) return true\n      } else if (event.action == android.view.KeyEvent.ACTION_UP && khatyarVolumePttActive) {\n        khatyarVolumePttActive = false\n        KhatyarRadioPttBridge.emit(this, \"up\", \"volume_up\")\n        return true\n      }\n    }\n    return super.dispatchKeyEvent(event)\n  }\n}`;s=s.replace(/\n}\s*$/,methods)}else{const javaMethods=`\n\n  private boolean khatyarVolumePttActive = false;\n  @Override public boolean dispatchKeyEvent(android.view.KeyEvent event) { if (event.getKeyCode() == android.view.KeyEvent.KEYCODE_VOLUME_UP) { if (event.getAction() == android.view.KeyEvent.ACTION_DOWN) { if (event.getRepeatCount() == 0 && !khatyarVolumePttActive && ${radioPkg}.KhatyarRadioPttBridge.INSTANCE.volumePttAllowed(this)) { khatyarVolumePttActive = true; ${radioPkg}.KhatyarRadioPttBridge.INSTANCE.emit(this, \"down\", \"volume_up\"); return true; } if (khatyarVolumePttActive) return true; } else if (event.getAction() == android.view.KeyEvent.ACTION_UP && khatyarVolumePttActive) { khatyarVolumePttActive = false; ${radioPkg}.KhatyarRadioPttBridge.INSTANCE.emit(this, \"up\", \"volume_up\"); return true; } } return super.dispatchKeyEvent(event); }\n}`;s=s.replace(/\n}\s*$/,javaMethods)}fs.writeFileSync(file,s,'utf8')}}
    return c;
  }]);
  return config;
}
